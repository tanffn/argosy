...
_PACKET = {
    ...
    "known_symbols": {"EXUS", "SPMV"},
    "plan_menu": [],
    "nvda": {"pct": 60.0, "cap_pct": 30.0},
    "reserve": {"shortfall_usd": 0.0},
    "instrument_facts": [],
    "policy_signals": {},
    "user_constraints": "",
}
...
def test_reliable_path_uses_real_author_dispatch_and_parses_model_output(monkeypatch):
    """Run wrapper -> real agent -> BaseAgent parser; stub only external seams."""

    async def fake_call(self, *, system, user, **kwargs):
        assert "deployment" in system.lower()
        return ModelCall(
            text=json.dumps(_good().model_dump(mode="json")),
            tokens_in=10,
            tokens_out=10,
            model="test-model",
        )
    ...
    monkeypatch.setattr(
        "argosy.services.agent_report_persistence.persist_agent_report_sync",
        lambda *args, **kwargs: None,
    )
    out = authored_allocation(
        _PACKET,
        user_id="ariel",
        config=ReliabilityConfig(hard_timeout_s=5, retries=0),
        breaker=CircuitBreaker(),
        cache={},
        verify=lambda proposal, packet: GateReport(status=GateStatus.ACCEPT),
        max_revisions=0,
    )
    assert out.status == "accepted"
    assert out.proposal is not None
    assert [buy.symbol for buy in out.proposal.buys] == ["EXUS", "SPMV"]
...
def test_accepts_and_caches_by_packet_hash():
    ...
    cache: dict = {}
    out1 = authored_allocation(
        _PACKET, user_id="ariel", run_author=run_author, cache=cache, breaker=CircuitBreaker()
    )
    assert out1.status == "accepted"
    assert calls["n"] == 1
    # Second identical packet → served from cache, no new author call.
    out2 = authored_allocation(
        _PACKET, user_id="ariel", run_author=run_author, cache=cache, breaker=CircuitBreaker()
    )
...
def test_breaker_opens_after_threshold_and_short_circuits():
    ...
    authored_allocation(
        _PACKET, user_id="ariel", run_author=run_author, breaker=br, config=cfg, cache={}
    )
    authored_allocation(
        _PACKET, user_id="ariel", run_author=run_author, breaker=br, config=cfg, cache={}
    )
...
def test_run_author_with_timeout_kills_on_hang():
    ...
    with pytest.raises(AuthorTimeout):
        _run_author_with_timeout(
            ...
            None,
            hard_timeout_s=0.2,
            killer=lambda: killed.__setitem__("n", killed["n"] + 1),
        )
