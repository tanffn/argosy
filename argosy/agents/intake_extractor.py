    ...
    def build_prompt(
        ...
        *,
        ...
        system = (
            ...
            "7. When the plan or attached documents describe RSU grants (e.g. an "
            "NVIDIA RSU portal screenshot, grant letters, payslip RSU lines), "
            "write the structured form into `identity_yaml` under "
            "`rsu_grants.grants[]` with one entry per grant carrying these "
            "fields: `award_id`, `award_date` (YYYY-MM-DD), `quarterly_shares` "
            "(integer; the recurring quarterly amount), and any free-text "
            "`note`. Do NOT bury this in `rsu_vest_schedule.active_grants` "
            "alone or in free-text notes — the gap tracker looks at "
            "`identity.rsu_grants.grants[]` to decide whether RSU intake is "
            "complete. You may ALSO populate `rsu_vest_schedule` with "
            "vest-date specifics; the canonical structured field is "
            "`rsu_grants.grants`.\n\n"
            "OUTPUT JSON SCHEMA:\n"
