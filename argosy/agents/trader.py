...
_FALSIFIER_RULE = (
    ...
    "  - **EVERY FALSIFIER / TRIGGER CARRIES AN EVIDENCE CONTRACT.** Never "
    "write a bare condition — author each one so a checker can adjudicate it "
    "without guessing. State, inline in the falsifier text (or in reasoning "
    "when a schema field cannot hold it), all six: (1) the METRIC and its "
    "SOURCE — which feed / filing / report line supplies it (e.g. '10-Q "
    "cash-flow statement', 'company data-center segment disclosure'), so it is "
    "checkable and not fake-precise; (2) an AS-OF DATE for the baseline you "
    "checked; (3) the CURRENT BASELINE value you observed; (4) an exact numeric "
    "THRESHOLD on the deteriorating side of that baseline; (5) a PERSISTENCE "
    "requirement wherever a single reading is noise (e.g. 'two consecutive "
    "quarters'); and (6) the DECISION TRANSITION the breach causes — one of "
    "HOLD->SELL, HOLD->ADD, SELL->PAUSE (stop reducing), SELL->ACCELERATE "
    "(reduce faster), BUY->EXIT, or ADD->HOLD. Example: 'Data-center revenue "
    "growth (source: company segment disclosure; as-of Q2 FY2027, currently "
    "+56% YoY) falls below +20% YoY for two consecutive quarters -> "
    "HOLD->SELL'.\n"
    "  - **DECLARE THE TRANSITION ON THE TYPED TRIGGER.** When you emit a "
    "typed revisit_trigger, set its OPTIONAL ``decision_transition`` field to "
    "the same transition the breach causes (HOLD->SELL / HOLD->ADD / "
    "SELL->PAUSE / SELL->ACCELERATE / BUY->EXIT / ADD->HOLD), so the sweep "
    "records what the tripwire would DO, not merely that it fired.\n"
    "  - **ON A SELL / TRIM, A FALSIFIER IS WHAT MAKES YOU STOP REDUCING — NOT "
    "WHAT MAKES THE SELL MORE URGENT.** For a REDUCE verdict the true falsifier "
    "is the condition that would INVALIDATE the reduction and PAUSE it (a "
    "SELL->PAUSE transition) — e.g. 'concentration falls back within the plan "
    "target band -> SELL->PAUSE (stop trimming)'. Conditions that make the "
    "reduction MORE urgent (deteriorating free cash flow, cut guidance, "
    "softening AI demand on a concentration-driven SELL) do NOT falsify the "
    "sell — they CONFIRM it: they are ACCELERANTS (a SELL->ACCELERATE "
    "transition) and MUST be LABELED as accelerants, NEVER listed among the "
    "falsifiers. Every SELL/TRIM verdict MUST carry at least one true "
    "PAUSE-falsifier (the condition under which you stop reducing); listing "
    "only accelerants as 'falsifiers' is the exact error the NVDA SELL made "
    "and is not allowed.\n"
    "  - **ETF / FUND FALSIFIERS NEED A NUMERIC THRESHOLD OR AN HONEST "
    "STRUCTURAL TAG.** For a fund, prefer an observable numeric threshold — "
    "expense ratio, index-methodology-driven yield or quality metric, tracking "
    "error, a distribution cut — carrying the same evidence contract. When NO "
    "numeric threshold is meaningful (e.g. SCHD 'reconstitution toward lower "
    "quality'), TAG the falsifier as 'STRUCTURAL:' and name the observable "
    "event that would evidence it (a methodology-change filing, a named "
    "holding-quality shift) — so it is HONESTLY flagged as qualitative rather "
    "than dressed up fake-precise or left un-adjudicable.\n"
    "  - **CITE FISCAL PERIODS CORRECTLY AND QUALIFY A KEEP-BUT-MIGRATE "
    "NAME.** Verify each fiscal-period label against the source before writing "
    "it — do NOT guess the fiscal year (e.g. NVDA's mid-2026 quarter is 'Q2 "
    "FY2027', not 'FY26 Q2'). And when a holding is economically sound but "
    "slated for replacement for tax/estate reasons (e.g. the SCHD->FUSA "
    "estate-domicile migration), do NOT write a bare 'do not sell' that could "
    "be read as cancelling the planned swap — write 'keep for now; do NOT "
    "cancel the planned migration'.\n"
...
class RevisitTrigger(BaseModel):
    ...
    label: str | None = None
    # OPTIONAL declared decision transition the breach would cause, e.g.
    # "HOLD->SELL", "SELL->PAUSE", "SELL->ACCELERATE", "BUY->EXIT". Enum-ish but
    # kept as a free string for forward-compatibility. Backward-compatible: it is
    # OPTIONAL (pre-existing triggers without it still validate), and the
    # deterministic trigger sweep reads only kind/price/metric/op/value/date —
    # it ignores this key — so nothing downstream must change. Serialized via
    # ``model_dump(exclude_none=True)`` so it is absent unless the trader sets it.
    decision_transition: str | None = None
...
class TraderProposal(BaseModel):
    ...
    rationale_summary: str = Field(
        description="A TIGHT, client-facing verdict — an advisor speaking to "
        "the client, UNDER 100 WORDS (aim 60-90). Open with the CALL in plain "
        "terms ('Keep NVDA but keep trimming on plan pace' / 'Hold SoFi, don't "
        "add'), then ONE sentence on the core why (the decision driver) and — "
        "only if material — ONE sentence on the key risk. NO section scaffold "
        "(no 'Quality read / Price read / Thesis fit / Data quality / "
        "Recommendation / Sources' headers), NO metric dump: the specifics "
        "(numbers, thresholds, sources, persistence) live in the FALSIFIERS; "
        "the deep analysis lives in the analyst reports the user can drill "
        "into. Plain English, honest, non-hyping. Put citations in "
        "``cited_sources`` (the list field), NOT inline in this prose."
    )
        ...
        *,
        ...
        if mode == "long_hold":
            system = (
                ...
                "  - Use short sentences.\n"
                "  - **``rationale_summary`` IS AN ADVISOR SPEAKING TO "
                "THE CLIENT — UNDER 100 WORDS (aim 60-90).** Write it "
                "the way you'd tell Ariel the call out loud: plain "
                "English, honest, non-hyping, like the home-greeting "
                "tone. Open with the CALL in plain terms ('Keep NVDA but "
                "keep trimming on plan pace' / 'Hold SoFi, don't add'), "
                "then ONE sentence on the core why (the decision driver) "
                "and — only if it's material — ONE sentence on the key "
                "risk. That's it. NO multi-section scaffold: do NOT emit "
                "'**Quality read:**', '**Price read:**', '**Thesis "
                "fit:**', '**Data quality:**', '**Recommendation:**' or "
                "'**Sources:**' headers, and do NOT dump every metric. "
                "The SPECIFICS (numbers, thresholds, sources, "
                "persistence) live in the FALSIFIERS you author below; "
                "the deep analysis stays in the analyst reports the user "
                "can drill into. If the user needs more, they will ask.\n"
                "  - **CITATIONS GO IN ``cited_sources`` (the list "
                "field), NOT in the prose.** Report names (e.g. "
                "'fundamentals/TICKER') and URLs. Do NOT embed bracket "
                "citations, a '**Sources:**' line, or raw URLs inside "
                "``rationale_summary``; the prose must read cleanly to a "
                "human.\n\n"
                "Rules:\n"
                ...
                "are required — in the ``cited_sources`` list field, "
                "never in the prose or mid-sentence.\n"
            ...
            system = (
                ...
                "**``rationale_summary`` IS AN ADVISOR SPEAKING TO THE CLIENT — "
                "UNDER 100 WORDS (aim 60-90).** Open with the CALL in plain "
                "terms ('Keep NVDA but keep trimming on plan pace' / 'Hold "
                "SoFi, don't add'), then ONE sentence on the core why (the "
                "decision driver) and — only if it's material — ONE sentence on "
                "the key risk. NO multi-section scaffold ('Quality read / Price "
                "read / Thesis fit / Data quality / Recommendation / Sources' "
                "headers) and NO metric dump: the specifics (numbers, "
                "thresholds, sources, persistence) live in the FALSIFIERS you "
                "author; the deep analysis stays in the analyst reports the "
                "user can drill into. Plain, honest, non-hyping. Citations go "
                "in ``cited_sources`` (the list field), never in the prose. If "
                "the user needs more, they will ask.\n\n"
                "Rules:\n"
