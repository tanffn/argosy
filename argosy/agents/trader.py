...
_FALSIFIER_RULE = (
    ...
    "  - **TRIGGERS MUST ENCODE THE FULL FALSIFIER, OR THE FALSIFIER STAYS "
    "QUALITATIVE.** For each mechanically-checkable falsifier, pair a typed "
    "revisit_trigger (see schema): fundamentals use kind=metric_condition with "
    "metric/op/value; a catalyst date uses kind=dated_event with an ISO date "
    "and a label. The trigger MUST fire on the SAME condition as its prose — "
    "never a weaker or different one. If the prose carries a duration, a "
    "segment, or a compound condition that metric/op/value cannot capture "
    "(e.g. '3+ consecutive quarters', 'data-center-led growth below 20%'), do "
    "NOT emit a lossy numeric trigger that would misfire: either name a "
    "segment-specific metric AND put the full qualifier in label, or leave "
    "that falsifier qualitative with NO trigger. BAD: prose 'data-center "
    "revenue growth below 20%' paired with metric_condition(metric=revenue_"
    "growth, op=<, value=20) — that fires on consolidated growth, the wrong "
    "signal. BAD: prose 'FCF negative for 3+ quarters with no revenue "
    "re-accel' paired with fcf_ttm<0 — drops the duration and the revenue "
    "condition; keep it qualitative instead.\n"
