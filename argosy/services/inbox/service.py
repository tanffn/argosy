...
def _cash_buy_list(db: Session, user_id: str, excess_usd: float, today: date) -> list[dict[str, Any]] | None:
    ...
    failure so the caller falls back to the detector's own proposals."""
    from argosy.services.period_directive import build_buy_list

    return build_buy_list(db, user_id, excess_usd, today)
