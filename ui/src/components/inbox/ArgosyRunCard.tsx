...
function stageLabel(stage: string): string {
  return ({
    not_run: "Not run", no_action: "No trades proposed", blocked: "Stopped", broken: "Broken linkage",
...
function ProofCheck({ check }: { check: E2EProofDTO["checks"][number] }) {
  const applicable = check.applicable !== false;
  return <div className="rounded border px-2 py-2 text-xs"><span className={applicable && check.passed ? "text-success" : "text-muted-foreground"}>{!applicable ? "N/A" : check.passed ? "✓" : "○"}</span>{" "}{check.label}</div>;
}

export { ManualFillForm } from "./ManualFillForm";
...
export function ArgosyRunCard({
  ...
  userId: string;
  compact?: boolean;
  /** A saved-review fallback must never become an actionable approval card. */
  noActionFingerprint?: string;
}) {
  const [fetchedProof, setProof] = useState<E2EProofDTO | null>(null);
  const reviewChanged = Boolean(noActionFingerprint && fetchedProof && (
    fetchedProof.artifact?.fingerprint !== noActionFingerprint
    || fetchedProof.lines.length !== 0
    || !["no_action", "blocked"].includes(fetchedProof.stage)
  ));
  const proof = reviewChanged ? null : fetchedProof;
    ...
    <Card className={compact ? "" : "border-primary/50"} data-testid="argosy-e2e-run">
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div><CardTitle>{compact ? (proof?.stage === "blocked" || proof?.stage === "broken" ? "Trade plan needs repair" : proof?.stage === "ready_to_accept" ? "Approve this trade plan" : "Trade plan status") : "Argosy run: decision → outcome"}</CardTitle><CardDescription>{proof?.headline ?? "Loading the persisted proof chain…"}</CardDescription></div>
      ...
      <CardContent className="space-y-4">
        ...
        {proof?.artifact && <>
          ...
          <details className="rounded-md border p-3"><summary className="cursor-pointer text-sm font-medium">{compact ? "Audit details and " : ""}NO-ACTION ({proof.no_action.length})</summary>
            {compact && proof.checks.length > 0 && <div className="mt-2 grid gap-2 sm:grid-cols-3">{proof.checks.map((check) => <ProofCheck key={check.key} check={check} />)}</div>}
            <ul className="mt-2 space-y-1 text-xs">{proof.no_action.map((row) => <li key={row.symbol}><strong>{row.symbol}</strong> — {row.reason}</li>)}</ul>
          </details>
          {compact && proof.lines.filter((line) => line.execution.manual_fill_allowed).map((line) => <ManualFillForm key={`${userId}:${line.proposal?.id}`} line={line} userId={userId} onSaved={refresh} />)}
        </>}
        ...
        {reviewChanged && <p role="alert" className="text-sm text-warning">The saved review has changed. Refresh the Inbox before making a decision.</p>}
      </CardContent>
    </Card>
  );
}
