...
**Fresh session: start at `docs/handovers/HANDOVER.md`.** That is the ONE handover file — a living document, updated in place. **Do not create dated handover siblings**; the 33 that used to exist were consolidated into it on 2026-08-12 and deleted (recoverable via `git log --diff-filter=D --name-only -- docs/handovers/`). It carries current state, the open queue, working discipline, traps, and restart recipes. Highest-value open item: **regenerate the plan** — the current plan predates the 2026-08-08/09 restore and the newest draft was computed with NVDA @ 0%, because `plan_synthesis/inputs.py::_summarize_positions` still excludes the unmanaged Schwab NVDA from the analyst's input (precondition A, still unfixed). Verify open items against `git log` before acting — prior handovers repeatedly listed already-shipped work as open.
