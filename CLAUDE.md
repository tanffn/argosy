...
**Active work lives on branch `feat/trust-restoration` (unmerged).** Tracker with per-task checkboxes: `docs/superpowers/plans/2026-08-13-trust-restoration-execution.md`; design `specs/2026-08-12-trust-restoration-design.md`; audit of all 94 design docs vs code: `docs/superpowers/INDEX.md`. The pass closes one root cause — **three guardrails reported success when they had not run** (codex math gate, whole-artifact reader, weekly digest). `argosy/quality/verification.py` now makes `DID_NOT_RUN` block promotion.
