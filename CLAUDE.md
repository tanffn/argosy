...
**Fresh session: start at `docs/handovers/2026-07-03-deployment-author-last-mile.md`** — the prime directive is the **fleet-authors / determinism-verifies pivot** (a simple LLM prompt beat Argosy's deterministic allocation; invert it). The last mile is SHIPPED to `master` and **PROVEN LIVE** against a real `claude.exe`: `DeploymentAuthorAgent` + decision-packet builder (now with per-sleeve current-vs-target gaps) + the P0 reliability wrapper (hard timeout + process-tree kill + circuit breaker) + `/deploy-cash` behind `deployment_author_enabled` (default off). NO tax pre-reserve (CGT is paid from the sale); plan-fit lives IN the author, never a gate. NEXT = UI for `dto.authored`, optional off-plan-position redeployment, api_key backend. Read memory `feedback_fleet_authors_determinism_verifies`, then skim `git log --oneline -25`.
