...
**Fresh session: start at `docs/handovers/2026-07-06-fleet-live-e2e-and-plan-v67.md`** — the fleet ran live end-to-end with real money: plan **v67 current** (x10 moonshot sleeve, IWQU replaced R1GR, gold OUT by model, NVDA 8 = Argosy's cap-derived seed); Ariel deployed **$161k across 11 broker fills** + a $20k SGOV cover-sale (the funding incident → the new deploy funding-breakdown feature); root causes fixed for the exit-1 "flake" (max_turns=1), the dead inbox sinks (kind CHECK), the cost under-billing, and the personal-config contamination (~69% input cut); analysts have WebSearch; verify-run skill is the post-run audit surface; snapshot self-refresh + apply-fills mean the TSV is OUTPUT-only. **Next:** bank DPYA reply → IWDP swap; next cash tranche EXUS-first; UI funding-actions render; SGOV migration + consolidation batch; the next real ingest must reconcile the armed closed-loop expectations. Read the handover §5 for the full open-items list, then `git log --oneline -25`.

Prior: `docs/handovers/2026-07-05-llm-team-architecture-and-fleet-reliability.md` — the team-over-gates direction correction, the deploy decision team, per-stock research→verdict, and the (since-resolved) exit-1 reliability blocker.
