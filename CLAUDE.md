...
**Fresh session: start at `docs/handovers/2026-07-07-scheduler-audit-sdd-refresh.md` (read the UPDATE block first)** — huge day: dead loop tier fixed (registry adoption — discovery/monthly/backup ran for the FIRST TIME EVER), phases A/B/C1 shipped (proactive 19:00 directive; closed-loop verifier armed on the $161k deploy; smart position-driven news), Discord disabled, SDD overhauled (41 divergences + NEW RULE: SDD is user-AGNOSTIC — no tenant plan values), FM first-greeting home rebuild in flight (agent). **REAL BUG found: plan synthesis dies under the server env — "claude.exe not found at ..._bundled" (run 136, 08:21 UTC); zombie 'running' decision_runs need the new reaper.** **Next:** finish/verify the greeting build; investigate the bundled-claude.exe synthesis failure; full-suite failures; watch the first full daily cycle then verify-run; bank DPYA reply; ~$9.6k tranche EXUS-first; next real ingest auto-verifies the armed expectations (phase B). Handover §4 open items, then `git log --oneline -25`.
