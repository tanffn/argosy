...
| T3.1 | Flip `phase5_agents` default on (gate off) | P3 | A0 | — | | ☑ | s16 f76120d — config default True (supervised live run); 3 real-fleet tests pinned to core (hermetic). 27 green |
| T3.2 | Wire EquityComp (RSU net savings + FV trajectory) resolver path | P3 | A1 | T1.6,T3.1 | ✓ | ☑ | s16 — resolver `_resolve_equity_comp_analyst` + render FV trajectory pre-existed + tested; gate flip (T3.1) connects them. **Codex-verified** money-math (known_grants_only floor + >25% spread→LOW). Live regen proves no [derivation pending] |
| T3.3 | Wire Withdrawal Sequencer (FI-bridge waterfall) resolver path | P3 | A2 | T1.6,T3.1 | ✓ | ☑ | s16 — resolver `_resolve_withdrawal_sequencer` + `_apply_fi_methodology` override + canonical_sections binding pre-existed + tested; gate flip connects them. **Codex-verified**. Live agent ran (LOW confidence — see report) |
| T3.4 | Wire real tax engine into MC; retire flat-10%/surtax-off shortcut | P3 | A3 | T1.6,T3.1 | ✓ | ☑ | s16 fd2b32c — `tax_curve.effective_withdrawal_tax_at_age` single source (15% pre-67 = CGT×gain-fraction / 12% post-67); `apply_age_aware_tax=True`; both inline 25/15/12 dups removed. **Codex-verified**. Headline: typical drawdown 46→47 (+1yr). 30 green |
| T3.5 | Wire PlanCoverageAnalyst output to a surface | P3 | A4 | T3.1 | | ☑ | s16 767a21d — `render_plan_coverage_appendix` reads the persisted `plan_coverage` report → "Coverage self-assessment" appendix (unfilled sections + missing_data); wired into render_plan_appendices. 13 green |
...
| T5.7 | Single tax-band source (drop triplicated 0.25/0.15/0.12, add surtax) | P5 | C7 | T3.4 | ✓ | ☑ | s16 ba424f4 — CGT(0.25)+gain-fraction(0.6) consolidated to tax_curve (was triplicated across cashflow_projection+tax_engine); `annual_surtax` (3% ordinary / 5% capital, >₪721,560) wired into compute_tax + a `surtax` TaxBreakdown field on /tax/compute. **Codex-verified** (zigzag: §122 rental IS in the surtax base per §121b/ITA 05/2025). 43 green |
