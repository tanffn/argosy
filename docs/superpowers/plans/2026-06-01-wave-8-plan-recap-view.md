...
**Status:** scoping **rev 4 — ZIGZAG CONVERGED**. Codex zigzag rounds 1 + 2 + 3 applied. Wave 8 ready for user approval to start.
...
- **Direction-reversal guardrail** (codex zigzag): if an intermediate waypoint **reverses direction** relative to today's value and the eventual endpoint (e.g., current NVDA 64.9% → medium 70% → long 15%), the intermediate is **always collapsed** (skipped) in v1. No opt-out, no synthesizer schema extension — the assumption is that no current plan legitimately needs an intentional hold-then-cut pattern, and adding a field "just in case" violates YAGNI. Loud warning event logged when a target is collapsed so the user/audit can see WHY. If a real plan ever surfaces a legitimate intentional rise, we'll extend `SynthTarget` in a future wave.
...
1. **Direction-reversal default behaviour — RESOLVED** (codex rounds 1 + 3). When an intermediate waypoint reverses direction relative to today's value and the eventual endpoint (e.g., current NVDA 64.9% → medium 70% → long 15%), the matcher **always collapses** the intermediate in v1. No opt-out, no synthesizer schema extension — YAGNI on a field that has no use case yet. Loud warning logged for every collapsed target so the audit trail captures the decision. If a real plan ever surfaces a legitimate intentional rise, we extend `SynthTarget` with the opt-out field in a future wave. Locked. (This resolves the original Q1 + the conflicting Q4 codex flagged in round 3 — both pointed at the same `intentional_hold_or_rise` field; v1 has none of it.)
...
4. ~~**Piece B1 — where `intentional_hold_or_rise` lives** (codex round 2).~~ **WITHDRAWN** per codex round 3: this question contradicted Q1 above. Final decision (Q1) is to skip the schema extension entirely; there's no `intentional_hold_or_rise` field in wave 8. Question removed.
