...
| Newest draft | `synth-2026-08-08-2200` — **computed with NVDA @ 0%, do NOT accept** |
| Backup | `scripts/backup_to_sibling.ps1` → `D:\Projects\financial-advisor-backup` |

**Book (post-restore truth):** a 2026-07-13 Leumi-only-import bug erased $2.4M / 59% of the book (incl. NVDA); it was restored on production 2026-08-08/09. True headline numbers: **NVDA ~58-60% weight, US-situs estate ~$1.28M**. Any synthesis that reports NVDA near 0% is wrong — that's the live defect below.
...
`1f6ca68` fixed the *resolver* (the final rendered number) but not the analyst's input. `argosy/orchestrator/flows/plan_synthesis/inputs.py::_summarize_positions` (defined :1287, comment at :1310) still keeps the summary "focused on tradeable holdings", so the **unmanaged Schwab NVDA is excluded from the ConcentrationAnalyst's input**. Proof: run 284 logged *"no NVDA position in the Leumi tradeable snapshot; current NVDA weight 0.0 UNKNOWN."* Until this surfaces unmanaged-but-present NVDA, a fresh synthesis reasons on 0.0 again and the resolver only cosmetically corrects the headline. **Fix before firing any regen.**
