...
**Last edit:** 2026-06-01 (night, wave 8 Piece A) by Claude — **post-Accept-All /plan routing fix: explicit five-state discriminator replaces the implicit JSX fall-through that landed users on the rejected #61 draft view after #62 was accepted**. Branch `main` @ `<piece-A-commit>` (one commit past `cf50509`). Wave 8 of the recap-view rollout — see plan doc `docs/superpowers/plans/2026-06-01-wave-8-plan-recap-view.md` rev 4 (ZIGZAG CONVERGED). Files touched: new `ui/src/lib/plan-view-state.ts` (pure discriminator returning one of `no_plan | pending_draft_triage | in_flight_synthesis | recap_current | stale_fallback_with_warning`), new `ui/src/lib/__tests__/plan-view-state.test.ts` (12 tests — 6 per-branch + 6 precedence including the exact #61/#62 regression: plan=19 + draft=18-superseded → recap_current), modified `ui/src/app/plan/page.tsx` to compute `viewState` once and gate every render block on it. Header subtitle also now suppresses the misleading "last draft #18" leak when the recap is active. Codex single-dispatch review of the discriminator verdict: **COMMIT AS-IS** with two minors (variant-role regression test added inline; integration-render test deferred to a later piece). Q3 default (pending-draft wins over recap when both exist) applied without escalation per the prime-directive's deterministic-precedence carve-out. Verification: backend curl confirms plan_version=19 canonical + draft fallback to #18 superseded + no in-flight (the exact #61/#62 scenario); discriminator unit-tested with that exact shape; SSR'd HTML carries `data-view-state` attribute; lint + typecheck clean on touched files; vitest 14/14 (12 plan-view-state + 2 baseline). **Next pickup is Piece E** (replace `<pre>` markdown dump with proper `<Markdown>` rendering for horizon_long_md / horizon_medium_md / horizon_short_md inside the recap layout — smallest remaining piece, ~half day). Then Piece G (headline + recap-summary service), then the chart pieces (B1/B2/F/C/D).
