...
- Commits #13+#14 (pending, bundled): Stage 1 deterministic news extractor + RSS + macro feeds (commit #13) + Stage 2 `news_signal_analyst` Opus agent (commit #14). Codex BLOCKER #2 isolation contract verified — Stage 1 has zero LLM imports, Stage 2 prompt construction never includes `raw_text` (the BLOCKER #2 canary test asserts a sentinel string + a prompt-injection payload + a non-whitelisted `$SHITCOIN` cashtag all stay out of the rendered prompt). New filename `news_signal_analyst.py` avoids collision with the existing Phase-2 `news_analyst.py`. Codex APPROVE_WITH_CONDITIONS on #13 — 2 IMPORTANTs fixed: (a) ticker extraction now case-insensitive (regex `[A-Za-z]{1,5}` + whitelist intersection) so informal sources writing "nvda" surface the symbol; (b) per-row SAVEPOINTs in `news_ingest.py` replace the per-loop `session.rollback()` that was undoing previously flushed rows on a single failure. 24 tests pass across extractor/ingest/analyst.
