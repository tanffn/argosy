...
| Deploy idle cash to the plan (risk-tiered, estate-aware buy list) | `argosy/services/deployment_advisor.py` (`assemble_deployment_plan`) + `argosy/services/high_potential_sleeve.py` (the moonshot sleeve) + `argosy/services/deployment_funnel/` (reconcile + flag facts) + `argosy/services/deployment_funnel/fleet_review.py` & `argosy/agents/deployment_disposition.py` (the fleet decides — per-line verdicts + the affirmative disposition) | §20.5; `GET /api/portfolio/deploy-cash` (`?fleet_review=true` = phase 2); `ui/src/components/proposals/DeployCashCard.tsx`; spec `docs/superpowers/specs/2026-06-12-deployment-advisor-design.md` |
| Deploy funding breakdown (WHERE the money sits + required pre-trade actions) | `argosy/services/deployment_funding.py` (`derive_cash_funding`) → `FundingBreakdownDTO` on `DeploymentPlanDTO.funding` (`argosy/services/contracts.py`) | §20.5; rendered by `FundingBlock` in `ui/src/components/proposals/DeployCashCard.tsx`, above the buy list |
| Snapshot self-refresh / apply broker fills (TSV is OUTPUT-only) | `argosy/services/snapshot_refresh.py` (`refresh_portfolio_snapshot`, `apply_fills_to_snapshot`) + `argosy/services/jobs/snapshot_refresh_job.py` | §20.4; `argosy/services/portfolio_ingest/tsv_generator.py` (`generate_family_finances_tsv`); job registered `enabled=True` in `argosy/api/main.py` (daily 08:00; manual Run-now uses the same tick) |
| Closed-loop fill verification (verify armed broker-fill expectations against the next real ingest) | `argosy/services/closed_loop.py` (`verify_and_record_on_ingest`, `sweep_unverified_expectations`, `collect_armed_expectations`) | §20.4; hooked from `argosy/services/portfolio_ingest/xls_osh_pair._write_through_resolved_snapshot`; swept daily by `pending_reevaluation_daily` |
| Fleet call reliability (transient retries, breaker, hard timeout) | `argosy/services/fleet_reliability.py` (`call_reliably_sync`, `CircuitBreaker`, `is_transient_fleet_error`) | §3.10; per-scope presets `CONSULT_ANALYST_CONFIG` / `DEPLOY_REVIEWER_CONFIG` |
| Verify a live run post-hoc (roster / degradation / groundedness / delivery) | `.claude/skills/verify-run/SKILL.md` | §14.6; reads `db/argosy.db`, `logs/app/application.log`, transcripts |
| Holistic rebalance / sell review (trim over-target to fund under-target, thesis-gated) | `argosy/services/holistic_rebalance_review.py` (`compose_rebalance_review` / `run_holistic_rebalance_review`) | §20.6; `POST /api/portfolio/rebalance-review`; quarterly `argosy/orchestrator/loops/holistic_rebalance_review.py`; `ui/src/components/proposals/RebalanceReviewCard.tsx` |
...
- **`account_class="limited"`** is the DB string for the Argonaut account. The feature/UI name "Argonaut" and the DB column value "limited" are different. Every site that persists an Argonaut proposal must write `"limited"`, not `"argonaut"`, because the broker router (`argosy/execution/router.py:102`) checks `proposal.account_class == "limited"`. Don't write `"argonaut"` to the column.
...
- **`decision_runs.tier` column is shared** across trade-flow (`"T0"`/`"T3"`) and amendment-chat (`"small"`/`"medium"`/`"large"`). `decision_kind` discriminates. A migration widened the column from `String(4) NOT NULL` to `String(8)` nullable.
...
- **`_persist_agent_reports` returns `list[int]`** — the inserted `agent_reports.id` values, in input order — so the caller can hand them to `record_negotiation_phase` for participant linking. Earlier code returned `None`; if you find a call site still ignoring the return, it's likely missing a recorder call.
...
- **Sell** is categorised by *why it exists*, not merely time-gated. The glide is the base **policy** sell; on top sits an **exception protocol** (`argosy/services/nvda_policy_sell.py`): a **thesis-break** — the NVDA thesis flagged BROKEN at critical severity by the thesis monitor — accelerates the trim to the cap *now* (the full over-cap sale) rather than pacing it over quarters; a WEAKENED flag holds the glide pace but surfaces a review note (no acting on a soft signal); an unverifiable thesis read holds the pace but is flagged *unverified*, never treated as all-clear. A **risk-budget** sell (`nvda_risk_budget.py`) is objective-based and *independent of the cap*: when a plausible NVDA drawdown (default 40%) would push net worth below the FI floor (the perpetuity base), it sells the smallest amount that restores the floor under that shock — so an even under-cap position can trigger a sell when it threatens the safe-retirement plan. A **catch-up** sell (`nvda_catchup.py`) repairs *missed scheduled tranches* — measured on execution (deconcentration-marked sells actually executed, by amount ÷ tranche size) vs the glide waypoints due by today, never the price run-up (that is risk-budget's concern). Each category converts to a recommended sale and the *largest* justified sale is taken as one trade (precedence thesis-break > risk-budget > catch-up > policy), so the tax and the over-cap amount are never double-counted. The daily decision funnel's graded verdict feeds in as a **concurrence note** (a bridge), and once it is out of beta calibration its own graded sells surface directly (§20.8).
...
**Nothing hidden.** A capability that is built is never kept hidden behind a flag. If it is still being calibrated or improved it is **exposed as "beta"** and worked on in the open; when it needs more data before it can be trusted, the amount collected so far is *shown* (e.g. "beta — N decisions over M days"), never gated behind an invisible threshold. Shadow-mode "record but surface nothing" is the wrong pattern — the client cannot trust or steer what they cannot see. (Applied first to the decision funnel, §20.8.)
...
- **Known-transient-only retries.** `is_transient_fleet_error` recognizes exactly two fingerprints — the CLI exit-1 burst and timeouts (`FleetCallTimeout` / `TimeoutError`). Only those retry; a real error (schema failure, bad prompt) surfaces immediately. Backoff is long and exponential (`min(60s, 20s × 2^attempt)`, default 2 extra attempts) — the exit-1 mode is a bursty resource condition, not a per-millisecond race.
- **Per-scope circuit breaker.** One `CircuitBreaker` per call family (e.g. `consult_analysts`, `deploy_reviewers`): 3 consecutive transient failures trip it, 5-minute cooldown, half-open single trial after cooldown. Per-scope so an outage in one call family never silently disables another.
- **Sync hard timeout + process-tree kill.** `call_reliably_sync` runs the attempt in a `ThreadPoolExecutor` with a hard deadline (`FleetRetryConfig.hard_timeout_s`; the deploy-reviewer preset uses 240s); on overrun it raises `FleetCallTimeout` and `_kill_claude_children` reaps orphaned `claude.exe` descendants of THIS process only (psutil recursive children — never siblings or the parent).

**Turn budget.** `BaseAgent` passes `max_turns=3` to the SDK as the no-tools baseline. A 1-turn cap starves adaptive thinking of its continuation turn — the model sometimes finishes thinking without emitting final text in turn 1, and the CLI then exits 1 with "Reached max turns" on stdout (an invisible-to-stderr flake fingerprint). With `allowed_tools=[]` the extra turns can only complete the text. Agents with tools get at least 6 turns (tool use consumes agent-loop turns); attachment chunking raises the cap to `chunks + 2`.

**Session isolation (`anthropic.claude_code_isolated`, default `true`).** Fleet `claude_code` sessions are isolated from the developer's personal Claude Code configuration: the SDK call sets `setting_sources=[]` (no user CLAUDE.md / auto-memory / filesystem skills or hooks leak into an agent's context) and mirrors the per-agent allowlist into `tools=` so the CLI's ~14k-token default tool schemas are trimmed to what the agent may actually use. OAuth session credentials are not a "setting source" and are unaffected. Net effect: the per-call prompt drops from tens of thousands of contaminated cache tokens to the irreducible core — the agent's own system prompt + schema.

**True cost accounting.** `cache_input_tokens` / `cache_creation_tokens` are read from `ResultMessage.usage` on both backends and billed as their own price buckets (the live API's `input_tokens` is EXCLUSIVE of cache reads/writes — treating it as inclusive under-bills every cached call). The log line carries `effective_input_tokens = tokens_in + cache_input + cache_creation` — the true prompt size; `tokens_in` alone is only the uncached remainder.

**Multi-turn response selection (`BaseAgent._select_response_text`).** On multi-turn runs the model may narrate or plan tool use in earlier assistant messages and emit the JSON in the last one. The parse target is the **last assistant message when it alone satisfies the output schema** (probed via `_parse_output`); otherwise the joined turn buffer is the fallback. The retry gates (empty-output, malformed-JSON) and `ModelCall.text` probe the same selected text, so a run whose final message is clean JSON never burns a retry on its own narration. Boilerplate rule 5 backs this from the prompt side: the final message must be exactly one JSON object — narration and tool planning belong in earlier turns only.

**Grounded hallucination detection (`BaseAgent._detect_hallucinated_sources`).** Cited source ids are checked against the supplied sources, with two grounding classes that legitimately pass without an id match: a citation found **verbatim inside a supplied source's content** (e.g. an article URL carried in a news payload), and **URL-form citations from an agent whose allowlist includes WebSearch** (the allowlist cannot know live-web URLs in advance). Invented ids stay flagged; the detector marks rather than strips, so downstream review decides.
...
**Scheduler-registered jobs.** The concrete runtime job set is composed from two registration points: `argosy/orchestrator/scheduler.py::register_default_loops` (the core loops above plus `process_cooling` 60s, `reconcile` 30s market-hours, `backup` daily 03:00, `audit` Sun 19:00, `watchlist` daily 08:30, `speculative_monitor` daily, `discovery_funnel` daily, `fx_refresh` daily) and `argosy/api/main.py::_start_jobs_scheduler` (the jobs below). All are manually triggerable via `POST /api/jobs/{name}/run-now`; visibility via `/api/jobs`; loops registered only in the scheduler are adopted into the job registry at boot (`adopt_unregistered_loops`) so everything is auditable from one surface. **Missed-run catch-up:** on boot, when `scheduler_catchup_on_boot` (default on) is set, any cron-scheduled loop whose last scheduled fire was missed — and is no more than `scheduler_catchup_max_age_days` (default 7) old — is fired once, sequentially through a catch-up gate so LLM-heavy runs don't stampede; interval loops are exempt. **Sleep/wake catch-up:** the same sweep also runs without a restart: a wake-watchdog task (`scheduler_wake_catchup`, default on) heartbeats on the wall clock every `scheduler_wake_watchdog_interval_seconds` (default 60) and treats a gap exceeding the interval plus `scheduler_wake_gap_threshold_seconds` (default 180) as a system sleep/suspend, re-running the catch-up sweep on wake. Double-fire safety: the tick-state check happens inside the catch-up gate, loops whose scheduled fire is currently in flight are skipped (ticks are recorded only at completion), and the scheduled path carries a slot guard — a cron fire whose slot already has a tick recorded (the watchdog caught it up while the loop's asyncio timer was stretched by the sleep) is skipped rather than re-run.

**Backend process supervision.** The backend runs under `scripts/run_backend_service.py` — a supervisor that spawns uvicorn as a child and restarts it on non-clean exit (rapid-crash limit: 5 crashes / 120s, then it gives up loudly). `scripts/start_backend_detached.ps1` launches the supervisor detached and is **idempotent**: it exits 0 without spawning when a supervisor process already exists, so it is safe to invoke from both a shell and an automated trigger. Reboot survival: the script is registered as a per-user logon task (Windows Task Scheduler, `Register-ScheduledTask` with an at-logon trigger — no elevation required), so the full chain is: reboot → logon task starts the supervisor → boot catch-up fires missed cron slots; sleep/wake without reboot is covered by the wake watchdog above. Environment the backend needs at boot (e.g. `ARGOSY_EXPENSE_SAMPLES_ROOT` for statement/RSU reconciliation) must be registered as user-level environment variables — a logon-task process does not inherit a shell profile. Windows process-inspection note: `.venv\Scripts\python.exe` is a venv launcher stub that runs the base interpreter as a child with the same command line, so ONE logical supervisor+uvicorn stack shows as FOUR `python.exe` processes (two launcher/worker pairs) — this is normal, not a duplicate stack.

| Job | Schedule | Enabled default | What it does |
|---|---|---|---|
| `state_observer` | Daily 17:00 IDT | on | Emergent anomaly detection over state; writes `state_observer_*` monitor flags (§14.2.1) |
| `thesis_monitor` | Daily 09:00 IDT | on | Per-holding thesis intact/weakened/broken verdicts; `thesis_monitor_*` flags |
| `payslip_ingest` | Daily 06:30 IDT | on | Payslip pickup + §102 withholding reconciliation |
| `holdings_review` | Daily 17:30 IDT | on | Per-stock research → BUY/HOLD/SELL/TRIM verdict pass; canonical collective vehicles route to `holdings_coverage_sweep` and its fund-vehicle analyst instead of being misread as companies. Stocks are triaged to material positions, fresh news/price/thesis is fetched, and only actionable verdicts surface (HOLD stays silent). |
...
| `period_directive_daily` | Daily 19:00 IDT | on | The proactive "your move" push (§1.6), after the review chain: deterministic triage asks whether there is excess idle cash **or a fresh autonomous buy/sell recommendation**, then checks the open directive's funding/source freshness. Only then the deployment author (the same fleet-authors path and decision packet `/deploy-cash` uses) reconciles portfolio, market, discovery and funding judgments into ONE validated `allocate` proposal. Zero new cash is valid: buys must then be funded by independently verified after-tax net sale proceeds. Component proposals are cancelled with history after consolidation so they cannot be approved as competing voices; their ids stay in the open directive payload for refresh/audit. The scheduler does **not** create executable rows: accepting the unified proposal atomically materializes all lines into awaiting-human trade proposals. For a zero-new-cash switch funded from one exact sell account, buy custody is that same account; otherwise an explicit funding account is required. A quiet skipped day is success; an unavailable/conflicted author writes nothing. |
...
| `signal_streams_daily` | Daily 15:30 IDT | on | Fetches early public-data nominations through the shared `SignalStream` contract, writes scoreable predictions, and persists candidates before the 16:00 discovery pass (§20.10); each stream is failure-isolated and advances its durable cursor only on successful commit. LLM recipient-resolution calls persist cost, prompts, raw response, and structured output to `agent_reports` / `agent_reports_blobs`. |
| `discovery_funnel` | Daily | on | The heavy discovery pass (radar → quick estimator → fleet grading) with its own failure isolation; feeds the high-potential sleeve's cached BUY picks (§20.5). Fresh estimator and discovery-grader calls use the standalone-agent persistence seam, so they participate in the monthly cost guard and weekly self-audit. |
...
| `news_daily` | Daily | on | Two-stage daily news pipeline. **Stage 1** (deterministic, no LLM): per-name RSS fetch covers held **single stocks** resolved at tick time from the latest snapshot via the instrument reference's `structure` axis (ETFs/funds get index/macro treatment only — an ETF's news is its market's news); also ingests the macro calendar (`argosy/services/macro_feed.py`, a static curated list that warns loudly within 30 days of its final entry — a refresh mechanism is an open design item). **Stage 2** (Opus analyst): gated by a deterministic triage that decides WHETHER to spend LLM — fires only when Stage 1 persisted new signals, OR a held single stock moved ≥ the `news_volatility_move_pct` config threshold (env `ARGOSY_NEWS_VOLATILITY_MOVE_PCT`, default 4.0) with unanalyzed signals pending; the analyst does all judgment. A quiet day skips agent construction entirely and records `reason='no new signals'` in the output summary. The `job_runs.output_summary` gains `tickers` (tick-time snapshot resolution: `single_stocks` count + `funds_light_treatment` count), `stage2_gate` (the triage verdict including `fired`, `reasons`, `pending_unanalyzed`, `volatility_moves`), and — on quiet days — top-level `analyzed` and `reason` keys |
...
- LLM-bearing cadence loops must explicitly check the cost guard (`should_pause_non_routine`) before constructing an agent. The automatic portfolio/discovery/recommendation chain enforces this at signal streams, discovery, holdings review, decision funnel, period directive/order-sheet authoring, weekly/monthly/quarterly review, watchlist, audit, minute/hour/annual cadences; routine `daily_brief` and deterministic `process_cooling` are exempt. Deterministic broker reconciliation, fill capture, and prediction grading continue while agent spend is paused.
...
2. The intake route asynchronously calls `PlanDistillerAgent` (Opus 4.8,
...
sequenceDiagram
 ...
 K->>C: spawn subprocess; write JSON over stdin<br/>{ system, user, model, max_turns:3, allowed_tools:[],<br/> permission_mode: "bypassPermissions" }
...
5. **Cost shape per turn:** ~3 input tokens (the user's accumulated answers are tiny relative to the system prompt + schema) + 500-1500 output tokens for the structured response. ~$0.04 per turn at Opus 4.8 rates.
...
| POST | `/api/plan/refine` | Incremental plan refinement — edit, don't rebuild: preview (`dry_run=true`) or apply (`dry_run=false` → a staged `role='draft'` PlanVersion derived from the current plan; unknown nodes → 400). The promotion gate validates a refinement draft's headline numbers against the resolver manifest rebuilt from the **nearest synthesis ancestor's** decision run (walks the `derived_from_id` lineage up to 10 hops — real validation against that run's manifest, never a structural exemption; a draft with no synthesis ancestor fails closed). |
| POST | `/api/plan/rollback` | Revert the current plan to a superseded version (reversible promotion). |
| GET | `/api/plan/action-items` | The "What's on you to do" checklist (§20.7): dated short/medium-horizon actions from the pending draft (else the current plan), each with `how_to` / `done_when` guidance + a `content_fingerprint` + `acknowledged` flag. `window_days` (default 14); `include_acknowledged` (default true). |
| POST | `/api/plan/action-items/{item_id}/ack` | Mark one checklist item done (§20.7): upserts a `plan_action_acks` row with the client-supplied `content_fingerprint`. Idempotent. |
| DELETE | `/api/plan/action-items/{item_id}/ack` | Un-acknowledge ("undo done") one checklist item (§20.7): deletes the `plan_action_acks` row. Idempotent — a missing row is a 200 no-op. |
| **Decisions** | | |
...
| GET | `/api/portfolio/deploy-cash` | Deployment Advisor (§20.5): tiered, estate-aware plan to deploy idle cash toward the plan target. `cash_usd` optional (defaults to snapshot idle cash); `live=true` adds market context; `sleeve_pct` (default 5.0) / `use_high_potential` (default true) carve the high-potential ("moonshot") sleeve off the top into the `high` tier. |
| GET | `/api/portfolio/high-potential-sleeve` | Standalone high-potential sleeve sizer (§20.5): conviction-weighted blend (UCITS thematic core + single-name carve-out). `cash_usd`, `sleeve_pct` (default 5.0), `live_radar` (carve-out from the trend radar vs advisor seeds). |
...
| POST | `/api/portfolio/rebalance-review` | Holistic, plan-driven, news-supported whole-portfolio rebalance review (§20.6): deterministic composition (no LLM) that trims over-target sleeves to fund under-target ones, thesis-gated + estate-gated. `write_proposal` (default true) persists ONE `rebalance` ActionProposal; never executes. Returns `{review, proposal_written}`. |
...
## 16.5 Plan repair — deterministic self-heal, in place
...
- **Shared-category split.** When several labels share one snapshot category, today's single snapshot value is split among them proportional to their end targets, so t=0 doesn't double-count it. Untargeted snapshot bands flat-line at today's value and are labeled "unconstrained" rather than dropped.
...
- **How much is deployable** (context banner) — the continuous overage detector (`unallocated_cash_detector.py`, rendered by `UnallocatedCashCard`). Self-tuning: it fires when current cash exceeds the plan-target cash row by ~1.5× and renders nothing when there is no overage. This is the "free cash above plan target → here's how much to put to work" signal.
- **Where it goes** (THE actionable list) — the deployment advisor's tiered, estate-annotated BUY list (`deployment_advisor.py`, rendered by `DeployCashCard`). Prefilled from the detected idle cash; the user can adjust the amount and regenerate. Each line carries an `is_new` flag — new vs already-held in the aggregate tradeable book — surfaced as a **NEW / ADD** tag, and a per-line **Accept / Defer** control persisted through the `allocation_actions` flow (the same persistence `UnallocatedCashCard` uses, sharing the `unallocated_cash` action source) so each accepted/deferred line renders its decision pill inline.
- **Where it came from** (context) — the event-driven reconciliation (`WindfallCard`, rendered with `showProposals={false}`). Self-suppresses (renders nothing) unless a cash-position change is detected; when it fires here it contributes the §102 reconciliation + allocation-delta as CONTEXT only — its own long/medium/short proposal columns are suppressed on this surface (they render in full on the retirement surface). One actionable list on `/proposals`, not three.

**One canonical cash-deployment engine.** There is a SINGLE instrument-selection engine behind every cash-deployment surface: `allocation_engine.compute_allocation(.., AllocationMode.CASH_ONLY_DEPLOY)` (the `cash_only_deploy` path), reading the canonical instrument-level `TargetAllocationDoc` (the §20 symbol-level plan). The windfall allocator (`windfall_allocator.py`: `propose_allocations` / `_allocate_long_term_from_plan` / `_candidates_to_long_proposals`), the unallocated-cash detector (`unallocated_cash_detector.py`), the RSU-prevest planner, and `/deploy-cash` (`deployment_advisor.assemble_deployment_plan`) ALL source their long-term / buy instruments from that one engine via `windfall_allocator.propose_allocations_from_plan`. There is **no hardcoded class→instrument map**. The consequence is an enforced consistency invariant: for the same `(doc, holdings, cash)`, the windfall / unallocated long-term instrument set and per-symbol dollars equal `/deploy-cash`'s `core` tier instrument-for-instrument. When no canonical plan is accepted these surfaces **fail loud rather than fabricate**: `propose_allocations` requires `doc` (no fallback map), and the unallocated-cash detector surfaces the overage with an **empty buy list + an explanatory headline** ("accept a plan to get a plan-bound buy list") rather than inventing instruments.

**The deployment advisor (`deployment_advisor.py::assemble_deployment_plan`).** Turns a net-of-tax deploy amount + the current canonical plan + current holdings into a risk-tiered, estate-annotated buy list, by wrapping the deterministic `allocation_engine.cash_only_deploy` and annotating each buy. Tiers, in carve order, are `reserve` → `core` → `medium` → `high`. Every plan-bound buy from `cash_only_deploy` is a `core` line; the buy must be cash-funded (a BUY leg funded by anything other than cash fails loud — the cash-only path must never miscount a trim-funded buy against the entered amount). Each line is tagged through the estate gate (`validate_instrument_domicile`, via `build_estate_map`): `estate_safe`, `us_situs_sanctioned` (the NVDA sleeve), `us_situs_exposed` (RED), or `unstamped`. The planned-buys' estate exposure is split on the headline (`us_situs_exposed_usd` vs `us_situs_sanctioned_usd`) so the sanctioned NVDA sleeve is never conflated with real RED exposure. Per-line pacing carries **no market-timing bet**: `lump-now` is the neutral default for a long-hold investor; whether to pace a material deploy (lump vs DCA) is a fleet judgment fed the plan + live data at decision time, not a hardcoded VIX / S&P-vs-trend rule.

**Exposure-aware deployment (`exposure_attribution.py` + `cash_only_deploy(exposure_aware=…)`).** The engine reasons about *exposure*, not the plan's exact ticker: before opening a plan instrument it credits the instruments you already hold that cover that sleeve. `classify_plan_substitutes(doc, holdings)` attributes each held instrument to at most one sleeve using the curated `instrument_reference` (asset_class / sector / region — not raw snapshot labels, so factor sleeves like value/momentum/low-vol never collide and US-core is never credited to an ex-US sleeve), and separates *attribution* (does the holding cover the sleeve?) from *implementation equivalence* (should fresh cash buy it?). The sleeve's gap is then `target − held(plan ticker) − credited substitutes`, and the incremental gap is bought in the **held estate-safe substitute** (`topup` — add to what you own) or, when the only cover is US-situs / a single name, in the plan's estate-safe instrument (`migrate` — fresh cash never grows the US-situs holding; the wind-down is surfaced as a separate flag, not a silent parallel buy). A fully-covered sleeve gets no buy. Substitute buy-lines are classified by the sleeve they fill (via the `plan_target:` cite) so they stay plan-bound `core` with the substitute's own estate tag. The behavior is gated by `exposure_aware` (off preserves the legacy per-ticker behavior; the canonical builder turns it on for `/deploy-cash`, the inbox, and the period directive), and the overflow redirect (below) applies the same substitution so it, too, tops up held funds rather than opening the plan's diversifier tickers by name.

**The deployment funnel — reconcile + flag facts; the fleet decides (`argosy/services/deployment_funnel/`).** The advisor's proposed buy list is not auto-executed on judgment. The funnel enforces the owner contract: *deterministic code reconciles against the plan's own numbers and surfaces FACTS; it never invents the investment judgment — the agent fleet decides.* A deterministic preflight (`run_preflight_for_plan` → `classify_candidate` + `candidate_flags`) produces two kinds of output: (1) **hard invariants** (no judgment) — a class absent from the plan routes to a plan change (`REQUIRES_PLAN_CHANGE`); conservation is enforced by the sizer; (2) typed **facts** (`CandidateFlag`: NVDA look-through density vs the plan's single-name cap, reserve overfund vs the plan's reserve target, unverified look-through) that *warrant* judgment but are never verdicts. The concentration cap is read as a per-name density limit: an instrument no denser than the cap is plan-compliant; deconcentrating an over-cap book is a SELL decision the cash path never makes on its own.

The flow is **two-phase**:
- **Phase 1 (fast, no LLM).** A candidate with no material flag is a clean plan-fill and deploys; a candidate carrying a medium/high-materiality flag is marked `NEEDS_FLEET_REVIEW` (pending) with its facts surfaced; low-materiality flags are informational and do not hold the buy. No money decision is made by a hardcoded threshold. (When the fleet is unavailable, `classify_candidate` supplies a conservative fail-safe fallback disposition; it never overrides a fleet verdict.)
- **Phase 2 (explicit, `?fleet_review=true`).** The agent fleet decides, producing two outputs: (a) **per-candidate adjudication** — RiskOfficer (3 perspectives) + FundManager return a BOUNDED verdict (approve / cap-at-% / veto) per pending line (`fleet_review.adjudicate_*`; the agents judge the given candidate, never invent an instrument or a size beyond the buy notional); (b) the **affirmative disposition** (`DeploymentDispositionAgent`, fund-manager lens) — a recommendation covering the FULL deployable amount in which every dollar is either deployed into a specific sleeve, **held as cash WITH A STATED REASON**, sent to deconcentration (trim NVDA), or gated behind a plan change. Holding cash is a valid recommendation *only* when the fleet decides it for a reason — never a silent residue.

**Fail-closed.** Any agent error leaves a pending candidate HELD (never silently approved) and the disposition null; a single flaky risk-officer call is tolerated (a majority of the 3 must respond). Config: `deployment_funnel_enabled` (master; off → the raw advisor list), `deployment_funnel_shadow` (annotate-only vs re-rank the buy list), and `deployment_fleet_review_enabled` (master kill-switch) gated by the per-call `fleet_review` opt-in (the fleet call is expensive — minutes of live agent calls, so it is never on the fast path). The `DeploymentPlanDTO` carries `preflight` (per-candidate facts + verdicts) and `disposition` (the headline recommendation); `DeployCashCard` renders the disposition as **"Argosy's recommendation (fleet)"** above the per-line facts, with a **"Run fleet review"** action that triggers phase 2.

**Conservation invariants** (money-math, asserted, fail-loud): `deployed_total + undeployed_remainder == deploy_amount` (over-deploy raises; cash the engine cannot place against plan targets is surfaced explicitly as `undeployed_remainder_usd`, never silently dropped). A material remainder carries a caveat; sub-dollar drift does not.

**Funding breakdown — the deploy says WHERE the money sits (`deployment_funding.py::derive_cash_funding`).** A buy list sized against pooled USD cash is executable only if the cash is actually reachable at the executing broker in the right currency. The plan therefore carries a `FundingBreakdownDTO` (`DeploymentPlanDTO.funding`): one `FundingRow` per `(account, currency)` cash position — local balance plus USD equivalent at snapshot FX, using the same `is_cash_position` classifier as the deployable total, so `total_usd` reconciles by construction — and a deterministic `required_actions` list derived from the rows: a **negative balance** produces a cover-it-first action ("Leumi USD is NEGATIVE: convert ~NIS … to cover before anything else"); a deploy amount the **largest single USD pot can't cover** produces the concrete conversions/wires ("Convert ~NIS … → USD (~$…) at Leumi before executing"). When the funding pool differs from the deploy amount, a `note` labels the difference; nothing is silently netted. `DeployCashCard` renders the breakdown (`FundingBlock`) **above the buy list** — the client sees where the money sits and what must happen first, before what to buy: per-row local balance (exact, red when negative) + USD equivalent, the total, and the "Before executing" actions.

**The high-potential ("moonshot") sleeve.** When `use_high_potential` (default `True`) and `sleeve_pct` (default `5.0`) is positive, a `sleeve_budget = deploy_amount × sleeve_pct/100` is carved off the **top** of the deploy amount — bounded to `[0, deploy_amount]` so a misconfigured pct can't carve more than the entered cash — and `core`/`medium` are computed on the **remainder**, so the sleeve is never double-counted. The `high` tier is populated by `_high_potential_lines`, which feeds `build_high_potential_sleeve` (`high_potential_sleeve.py`) from **cached discovery BUY picks** (`_cached_buy_sleeve_candidates` reads the persisted discovery `ScanState` and keeps only graded picks whose verdict is BUY) with the seed-candidate set as the fallback. This path is synchronous and read-only — **the deploy path never triggers a live discovery-funnel run.** Each sleeve pick is sized by its conviction weight (HIGH=3 / MEDIUM=2 / LOW=1) renormalized across the budget — no hand-picked dollar figures — and run through the same estate gate (`_sleeve_estate_tag`): a UCITS thematic name tags `estate_safe`, an unsanctioned US single-name tags `us_situs_exposed` (the user's consciously-accepted estate hit on the convexity slice). The sleeve's estate split folds into the headline totals. A second conservation guard asserts the high-tier lines sum to `sleeve_budget` within $0.50. With `use_high_potential=False` (or `sleeve_pct ≤ 0`) the `high` tier stays empty and `core` is computed on the full amount.

**The sleeve mandate is x10-asymmetry-first (`high_potential_sleeve.py::X10_SLEEVE_MANDATE`).** The moonshot sleeve exists for convexity, not diversified quality-growth, and the binding mandate text is injected verbatim at every surface that grades or fills it — the discovery funnel's `QuickEstimatorAgent` and `DiscoveryGraderAgent` prompts, and the deployment-author packet (`allocation_author/packet.py`) so the author fills asymmetry-first. Its clauses: (a) a **cap-math test** — a plausible x10 needs a sub-~$20–30B starting cap; a >$50B name needs extraordinary justification; (b) the **accepted per-name loss is 100%** — defensibility must not boost rank; (c) **rank = upside-asymmetry × plausibility**, not conviction-safety; (d) **deploy fill order is asymmetry-first**. Domicile is deliberately sleeve-scoped: sleeves whose `sigma_class` is in `_DOMICILE_EXEMPT_SIGMA_CLASSES` (`high_growth_basket`) are exempt from the UCITS-preferred domicile gate **by sleeve membership, never by ticker** (`target_allocation_doc.check_domicile_violations`) — many moonshot names have no UCITS twin, the sleeve is small and managed as a basket, and the same ticker held in any other sleeve stays fully gated.

A separate `GET /api/portfolio/high-potential-sleeve` exposes the sleeve sizer standalone (its own `cash_usd` / `sleeve_pct` / `live_radar` params, the live carve-out sourced from the trend radar rather than cached discovery); the **collapsed** `high-potential` section on `/proposals` leads with the fleet-graded discovery surface, with the legacy radar/monitor cards behind a "Raw sourcing (advanced)" disclosure.

### 20.6 Holistic rebalance / sell review

`holistic_rebalance_review.py` composes ONE whole-portfolio rebalance proposal — trim over-target sleeves to fund under-target ones — gated by thesis strength. It is a **deterministic composition over existing engines**, not a new analysis: it writes NO new drift math, NO new per-position verdict logic, and NO new news pipeline. `compose_rebalance_review` is a pure function; `run_holistic_rebalance_review` loads the real inputs through existing accessors and composes them. The reused engines are:

- **Drift** — `rebalancing.detect_rebalancing_alerts` → per-coarse-class `RebalancingAlert` (the 5pp / 25%-relative rule).
- **Per-position verdicts** — `per_position_thesis.derive_position_theses` → HOLD/BUY/TRIM/SELL/ADD with current/target weights + conviction.
- **News / thesis** — active, unexpired `monitor_flags` of kind `thesis_monitor_*` and `alpha_report_caution`, keyed per ticker.
- **Estate gate** — `validate_instrument_domicile` (UCITS-preferred; NVDA the only sanctioned US-situs sleeve).

**Thesis-gating rule (trims).** Only positions whose coarse class (equity / bonds / cash) is flagged over-target are candidates. A held position is trimmed only when one gate fires: (a) `THESIS_OVERWEIGHT` — the per-position verdict is TRIM or SELL (the plan itself wants the weight down, e.g. intact-but-overweight or a UCITS domicile swap); (b) `THESIS_WEAKENED` — an active `thesis_monitor_weakened`/`broken` flag exists; or (c) `NEWS_CAUTION` — an active `alpha_report_caution` mentions the ticker. A high-conviction position with an intact thesis is **never** trimmed unless the class drift is CRITICAL — the 25%-relative rule fired OR absolute class drift ≥ 12pp — recorded as `CRITICAL_DRIFT_OVERRIDE`. A target-based trim (per-position target known) is sized exactly to that target and is NOT capped by the coarse class overage (several names in one class can each be over their own target while the class nets to a smaller overage). A fallback trim (no per-position target) is capped against a shared **per-class trim budget** = the class overage, so multiple fallback legs can't double-count the overage.

**Buy side.** BUY legs fund the most under-target classes from per-position ADD/BUY verdicts, sized to absorb the trim proceeds up to the under-target shortfall (so the review reads fund-this-from-that; net cash delta is reported). Every BUY candidate runs through the estate gate; an estate-RED (US-domiciled, non-sanctioned) candidate is **dropped** with a recorded note (use the Irish UCITS twin), never proposed. Every SELL/TRIM leg carries a taxable-event note (the consequence is cited, not computed — the realized-gains engine owns the exact liability). A TRIM never exceeds the position's current value.

**Persistence + fail-loud.** The review is always returned. It persists exactly ONE thesis-gated `ActionProposal` (`kind='rebalance'`) via `write_action_proposal` (tombstone-then-insert dedup; `execution_state='proposed'` hardcoded) and **never executes**. A missing canonical plan or an empty portfolio snapshot returns `status='cannot_review'` (fail-loud — never a silent empty review on a missing critical input). The proposal renders in the `/proposals` action-proposal list.

**Triggers.** On-demand via `POST /api/portfolio/rebalance-review` and the always-visible **"Run portfolio review"** card (`RebalanceReviewCard`) on `/proposals`; and scheduled via the quarterly `holistic_rebalance_review` monitor loop (`source_kind='monitor'`, 10:00 IDT on the 1st of Jan/Apr/Jul/Oct), registered in both `argosy/api/main.py` and `orchestrator/scheduler.py` so `argosy run` and the FastAPI server boot the same job set. It is manually triggerable via `POST /api/jobs/holistic_rebalance_review/run-now`. The loop's own dedup/cooldown + material-drift gating (it writes only when `status='ok'` and there are legs) make an off-cadence run safe and idempotent.

### 20.7 The plan action checklist — actionable + completable to-dos

`GET /api/plan/action-items` (`argosy/api/routes/plan.py`) backs the consolidated **"What's on you to do"** checklist on `/proposals`. `_collect_action_items` walks the user's pending draft (else the accepted `role='current'` plan) and emits one `ActionItem` per dated short/medium-horizon `action` — only actions carrying a parseable ISO date (`stated_at` / `due_date` / `trigger_or_date` under `horizon_kind='dated'`, else the earliest embedded `YYYY-MM-DD`) are eligible, since a directional or parameterized trigger has no calendar slot. Items are sorted by date (overdue first) and status-classified `OVERDUE` / `TODAY` / `DUE_SOON` (≤3 days) / `UPCOMING`. No current plan returns an empty list with 200, never 404.

**Actionable.** Each item carries a `how_to` (concrete steps, pointing at the relevant Argosy surface) and a `done_when` (a checkable completion criterion). These are **source-first**: the synthesizer's `Action` may emit optional `how_to` / `done_when` fields (`argosy/agents/plan_synthesizer_types.py`, default `""`), and when present they win. Otherwise the route falls back to a deterministic, keyword-keyed guidance mapper (`argosy/services/action_item_guidance.py::guidance_for_action`), matching the action's verb/keywords + `horizon_kind` against ordered categories (`verify_withholding`, `verify_contribution`, `verify_check`, `rebalance_trim_sell`, `buy_deploy_allocate`, `contribute_fund`, `convert_fx`, `harvest_tax`, `review_reassess`, `generic`). The mapper is side-effect-free and **never fabricates numbers** — concrete figures stay in the plan-authored label/detail; the guidance only supplies steps and a completion bar.

**Completable + resurface-on-change.** Each item carries a `content_fingerprint` — a stable SHA-256 (first 32 hex) of `item_id` + label + the dated ISO day + any monetary amounts found in the label/detail (deliberately ignoring rationale / cited_sources / status, which are cosmetic or time-derived) — and an `acknowledged` flag. `POST` / `DELETE /api/plan/action-items/{item_id}/ack` toggle a `plan_action_acks` row (one per `(user_id, item_id)`, UNIQUE so re-acking upserts; **migration 0073**). An item reads `acknowledged=True` only when an ack row exists AND its stored fingerprint matches the freshly recomputed one — so a plan edit that renames the item, moves its date, or changes an amount shifts the fingerprint, the prior ack stops matching, and the item **resurfaces as not-done**. `GET` takes `include_acknowledged` (default `true`): acknowledged items are returned with `acknowledged=True` so the UI can render them as done / allow undo; passing `false` drops them.

### 20.8 The decision funnel — the daily per-stock "act?" agent

`argosy/services/decision_funnel/` is the single daily driver behind the Proposals surface (§13 surface 4): a tiered escalation agent that converts fresh signals into fresh Buy/Sell/Hold proposals. It is a **conservative escalation system, not a daily recommender** — cheap-first, deep-only-where-it-matters, default NO-OP.

**The tiers** (`orchestrator.py::run_funnel`):
- **Stage 0 — market review** (`stage0_market.py`): a compact macro read off ALREADY-INGESTED data (latest `AlphaReportAnalysis` macro tone, active macro/volatility `monitor_flags`, recent high-materiality `news_signals`, best-effort VIX from `macro_cache`). No new fetches; source-cited; degrades to neutral.
- **Stage 1 — deterministic relevance routing** (`stage1_routing.py` + `policy.py`): maps the macro read + per-name signals onto the book (`book.py`, reusing the canonical tradeable-securities concentration denominator). A name must EARN a deep review. HARD TRIGGERS (thesis broken / weakened, big move, deep drawdown, imminent earnings, concentration-cap breach, NVDA drift-band breach) force a review bypassing cooldown; high-materiality news bypasses cooldown; otherwise the name DROPS (reason recorded, never silent). A pending/absent IPS cap routes `concentration_unverified` (never a silent drop). A deterministic, replayable AUDIT sample of drops (denser for blind/no-coverage names) catches false-drops. A risk-off macro read routes the broad-equity sleeves only. The policy is versioned (content hash) and stamped on every run.
- **Stage 2 — triage** (`triage.py`): a cheap Sonnet pass per routed candidate kills the no-ops.
- **Stage 3 — deep decision** (`deep_decision.py`): survivors run the full `decisions/flow.py` fleet at tier T2 (analysts → bull/bear → trader → risk → fund manager). **Propose-and-ask only** — never auto-executes a discretionary trade.

**The IPS** (`ips.py`) derives the policy the funnel reasons against (target risk, concentration caps, sell discipline, retirement horizon, tax priorities) from the current plan's canonical facts — no fabrication; pending fields are tracked, never guessed.

**North-star gate** (`north_star.py`): before a proposal reaches the client's "needs me now" surface, it must trace to the prime directive (a material trigger); audit/no-trigger proposals are recorded but not pushed.
...
### 20.9 The action-proposal inbox — sinks, dedup, supersession

The client-facing inbox is backed by `action_proposals`. The table's `kind` CHECK constraint (migration **0077**) admits `allocate`, `repatriate_currency`, `rebalance`, `replan_full`, `add_life_event_phase`, `update_plan_assumption`, `set_watchlist`, `note_only`, plus the two agent-decision kinds: **`stock_decision`** (the daily holdings-review's actionable BUY/SELL/TRIM verdicts — HOLD stays silent; `argosy/services/stock_decision/service.py::write_stock_decision_proposal`) and **`deploy_team_flag`** (the deploy decision team's judgment flags; `argosy/services/deploy_decision_team.py::write_team_flag_proposals`).

**Sinks fail loud, dedup in place.** Both sinks are idempotent per `dedup_key`, and a swallowed `IntegrityError` is always **logged** (`stock_decision.proposal_write_skipped` / the team-flag warning) — a constraint failure must never masquerade as a silent dedup collision. The deploy-team sink goes further: on a dedup collision it **refreshes the open row in place** (summary, rationale, suggested payload, severity, `surfaced_at`, `expires_at`), so a re-raised flag carries this run's facts rather than a stale first impression.
