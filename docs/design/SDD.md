...
**Pickup options for next session** (in plain-English outcome order — wave 8 is the user's stated new priority):

1. **★ Wave 8 — Plan recap view ("what's the plan, in plain English")**. User-stated priority after the Accept-All UX gap surfaced (synth #62 approved, plan_version=19 promoted, but /plan falls back to confusing stale draft view). Visual-first recap composing existing visualizations (NVDA trajectory chart + cashflow projection + Monte Carlo + ruin probability + target progress strips + 13 retirement cards) into a single "here's your plan" surface. Six pieces (A post-accept routing / B allocation glidepath / C synthesizer-aware cashflow defaults with rationale / D Monte Carlo integration / E markdown rendering / F cross-horizon actions timeline / G plain-English headline). Plan doc at `docs/superpowers/plans/2026-06-01-wave-8-plan-recap-view.md` (rev 4, commit `f8e294d`); **codex zigzag CONVERGED after 3 rounds**: round 1 SCOPE-CHANGES (7 findings — B split, C narrowed, direction-reversal guardrail, non-% exclusion, A state discriminator, 4 at-a-glance blocks); round 2 ZIGZAG-ESCALATE (3 contract gaps — C lifestyle_drift sourcing, timeline table B1/B2, G recap-summary contract; 3 new open-question surfaces); round 3 ZIGZAG-ESCALATE-FINAL (one remaining Q1/Q4 contradiction over the `intentional_hold_or_rise` schema field; resolved as "no schema change in v1, always collapse reversal waypoints"). Wave 8 ready for user approval to start implementation; ship order **A → E → G → B1 → F → B2 → C → D** so the page has something readable from day 1 and charts compose in. Outcome: a non-expert can look at /plan and understand what the plan IS — current allocation → planned glidepath over time, NVDA share trajectory, monthly cashflow with pre-populated sliders + rationale tooltips, Monte Carlo verdict, key-actions timeline, formatted plan markdown. ~1.5-2 weeks per timeline in the doc. **This is the next critical task per the user.**

2. **Refresh action sheet from #62 (now-current plan)**. One-page handout for "what to do this week" based on the approved plan; the FM's 8 tracked refinements become practical next-step items. Outcome: actionable today; doesn't require another synth loop. Existing sheet at `docs/handovers/2026-06-01-act-on-current-plan.md` was written against #61's content — may want a refresh against #62 + the 4 deltas Ariel just accepted (US-situs $1.37M target, FIRE-bridge analysis 49→60→67, broad-market multiple-compression risk theme, parameterized UCITS top-up trigger). Lower priority once wave 8 ships the recap view that surfaces this content visually.
3. **Wave 7 Piece B v1.5 — post-FM auto-populate**. When the next synthesis FM verdict commits, run the carry-forward matcher (already built + tested) against the new draft's objections + write `fm_objection_user_state` rows with the prior stance + counter-position + audit fields. /plan loads with stances pre-populated for the matched subset. Outcome: less click-work on /plan when iterating; doesn't help when FM approves first try.
4. **Wave 7 Piece A — triage agent (central wave-7 piece)**. New agent runs after phase 5; classifies each FM objection into auto_dispatch / defined_fix / human_in_loop / invalid_objection / dismissed. Plan ships APPROVED-WITH-NOTES when human_in_loop == []. Includes 4-stage defined-fix safety pipeline. Outcome: when FM raises concerns, Ariel sees only the items that genuinely need user judgment; the rest auto-resolve silently. ~3-4 days; the central convergence piece. Less urgent now that #62's FM verdict showed self-convergence via prompt-only guidance.
5. **Wave 6 — enforcement substrate (FM-OBJ #7 fix)**. Typed `PlanPolicy` with `TickerCapRule`/`SectorCapRule`/`ExposureCapRule`; internal `instrument_classification` table populated by scheduled Finnhub sync; `risk_preflight.check_concentration_cap` rewrite for typed + sector lookups; reliable distillation backfill via CLI + queue + startup health check. Outcome: system-enforced caps at trade time — actual protection when a trade would breach the 35% sector cap or 15% NVDA cap, not just plan prose. ~1.5-2 weeks per codex zigzag.

Note: the post-Accept-All /plan UX gap (stale fallback view + "loading verdict…") is no longer a "bonus follow-on" — it's absorbed into **wave 8 Piece A** above (post-accept state routing).
