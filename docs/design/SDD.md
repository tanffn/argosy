...
- Commit #7 (pending): Schwab parser extension + new `rsu_vest_events` table (migration 0044) + `argosy/services/rsu_vest_ingest.py`. **Spec revision**: the original draft (`rsu_unvested_grants` table for future-vest data) didn't match the real CSV — Schwab exports HISTORICAL vest events via paired Lapse + Deposit rows, not future-vest projections. Renamed table to `rsu_vest_events`; upcoming-vest projection moved to a separate computed-on-the-fly concern (commit #10 follow-on). The parser models Lapse rows as canonical vest events; Deposit rows are paired settlement events (T+1) and counted in `unparsed_actions` to avoid duplicate vest emissions. Codex review APPROVE_WITH_CONDITIONS — 2 IMPORTANTs + 1 NICE + 1 NIT fixed before commit: (a) observability counter `unparsed_actions['Lapse_no_continuation']` when a Lapse row's continuation is missing/interrupted; (b) first-write-wins semantics documented explicitly in `ingest_schwab_vest_events` docstring (no upsert on Schwab CSV reissue — operator deletes + re-runs if a corrected file lands); (c) new CHECK constraint `shares_withheld <= shares_vested` (cheap conservation invariant); (d) tightened test assertions from `>= 50` to exact 74 (catches regressions). 9 tests pass against the real fixture (74 vest events; grant 213000 on 2026-03-18 = 280 shares at $181.93 FMV; new Lapse-without-continuation test verifies counter fires).
