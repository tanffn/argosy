...
- Commit #10 (pending): `<HolisticTimelineCard>` on /retirement + backend `argosy/services/retirement_timeline.py` + `GET /api/retirement/timeline` route. Composes past vests (rsu_vest_events) + projected future vests (via the +90d quarterly heuristic from commit #9) + life events (filtered to dated rows) + three retire-ready zones (bear/base/bull from the canonical clamp function) into one payload. MAX_FUTURE_VESTS=12 caps projection count. UI: scrolling timeline with five overlay layers, percentage-positioned markers, 10y/30y horizon selector, today indicator, hover tooltips. Codex APPROVE_WITH_CONDITIONS — 2 IMPORTANTs addressed: (a) horizon_days now clamped [MIN_HORIZON_DAYS=1, MAX_HORIZON_DAYS=365*50] at the service layer to prevent negative-horizon zero-vest silent failures + DoS-via-huge-horizon; (b) 3x-scenario perf-duplication accepted for v1 with explicit comment (timeline is page-load not per-keystroke; <2s on real households; batched scenario projection is a follow-on). 1 NICE addressed: today-boundary test now asserts a vest dated exactly today classifies as past. 8 backend tests + 5 backend regression tests pass, TypeScript clean.
