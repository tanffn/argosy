...
- Commit #18 (pending): final user-guide refresh — reflects all IA changes this sprint. Net-new sections: /life-events page walkthrough, Holistic Timeline subsection on /retirement, /proposals#allocation subsection (relocated WindfallCard), Home Red-Flag Strip, Monitor agent overview, News pipeline overview. Updated sections: nav order (Life Events between Expenses and Plan; /consult replaced /decide), /portfolio (UnallocatedCashCard now has live Accept/Defer), /retirement (read-only visualization). TOC reordered. Hole #3 (XLS upload closed) removed. TV-manual voice per [[feedback_user_guide_is_manual]] — sanity-grep verified zero hits for "you're right" / "still holds" / "the reason we" / "/decide" / "retirement#windfall".
