...
|---|---|---|
| A | `0cc3dc8` | Five-state discriminator (`no_plan` / `pending_draft_triage` / `in_flight_synthesis` / `recap_current` / `stale_fallback_with_warning`) replacing the implicit JSX fall-through. 12 unit tests pin the matrix including the #61/#62 regression. Codex single-dispatch review: COMMIT AS-IS. |
| (stamp) | `bc34c13` | Stamped Piece A commit hash into SDD handover. |
| E | `907d655` | `<pre>` → `<Markdown>` for `horizon_long_md` / `horizon_medium_md` / `horizon_short_md` in PlanRevisionSheet + recap "Full plan" card; fetches `/api/plan/current/structured` to surface the per-horizon markdown. |
| G | `42f1cf9` | `argosy/services/plan_headline.py` + `/api/plan/current/headline` + `HeadlineCard` UI: three-line headline (retirement readiness / next big move / then) + four at-a-glance tiles (accepted deltas / portfolio total / insurance gaps / audit). 25/25 unit tests on the pure-function math layer. Verified end-to-end against plan_version=19: retire age 44 base, 1 accepted delta (US-situs $1.37M target), $3.8M portfolio, ltc + health-supplementary gaps, audit chain to run #62. |
| B1+F | `d8d3854` | Backend `argosy/services/allocation_glidepath.py` (waypoint stitching, direction-reversal collapse with loud-WARNING logging, pct-only filter, per-class pct-scale auto-normalisation) + `/api/plan/current/allocation-glidepath` route + UI ActionsTimeline component (cross-horizon, grouped into dated / parameterized / directional sections + excluded non-pct targets surfacing here so nothing gets dropped). 34/34 tests. Codex zigzag round 1 verdict: SCOPE-CHANGES-NEEDED (non-blocking) — all 3 findings (loud-warning assertion test, fractional-month boundary tests + duplicate-date test, no-snapshot-match warning telemetry) applied inline before commit. |
| B2+C+D | `863198c` | UI AllocationGlidepathChart (Recharts stacked-area with collapsed/excluded callouts) + backend `argosy/services/cashflow_assumptions.py` (deterministic v1, NO synthesizer-posture-string interpretation per zigzag-round-1 lock) + `/api/plan/current/cashflow-default-assumptions` route + UI AssumptionsCard (six fields, click-to-expand `▸ why?` tooltips, source-tagged: `sigma_calibrator` / `goals_yaml` / `default`) + `/api/plan/current/cashflow-monte-carlo` route alias + UI MonteCarloBandsChart (Recharts ComposedChart with P10–P90 + P25–P75 bands + median line + lump/annuity reference lines + traffic-light verdict tile). 18/18 cashflow_assumptions tests. Verified end-to-end: sigma auto-calibrates to 34.4% on plan_version=19 (NVDA-heavy concentration captured); MC returns 50-year projection with traffic-light verdict; recap renders the full layout under `viewState=recap_current` for plan_version=19. |

**Wave 8 final state**:
- Tests: 77 wave-8 backend (cashflow_assumptions 18 + allocation_glidepath 34 + plan_headline 25) + 14 UI vitest (12 plan-view-state + 2 baseline) all passing.
- Lint + typecheck clean on every file touched.
- Codex tandem footprint: 2 single-dispatch reviews (Piece A discriminator + Piece B1 service); both verdicts integrated inline (A = COMMIT AS-IS + 1 minor applied; B1 = SCOPE-CHANGES-NEEDED + 3 minors applied).
- The recap view renders fully against the live plan_version=19 + real portfolio snapshot. Headline shows retirement age 44 base case; glidepath shows 3 asset classes (all anchored at 0 because synth labels don't match snapshot categories — flagged via no-snapshot-match warning, deferred to v2); MC shows 0% failure before 95.

**Known v2 follow-ons (NOT blocking wave-8 close)**:
- The synthesizer emits asset-class labels as descriptive sentences (e.g. "info-tech sector cap on managed portfolio (excludes nvda rsu)") that don't match the portfolio_snapshot's short `AllocationRow.category` strings ("Growth", "Value", etc.). The glidepath fallback (0.0 t=0 anchor + WARNING log) is acceptable v1 per codex's review but the chart's anchoring is misleading. v2 fix: synthesizer emits a normalized `asset_class_key` alongside the prose label, OR a manual mapping table in the glidepath service.
- Codex Piece A MINOR #1: full integration render test asserting `data-view-state="recap_current"` post-hydration with the matching sub-component presence/absence checks for the #61/#62 scenario. Needs heavy mocking (7 API methods + WS hook + router). Deferred to a follow-up — the pure-function discriminator's 12 tests are the primary safety net.
- Piece C UI for v2: tightly integrate the AssumptionsCard's defaults into the existing CashflowProjectionChart's sliders so editing the slider value tracks back to "you overrode the calibrated σ of 34.4%". Today the AssumptionsCard is informational only; the chart sliders read their own defaults.

**Pickup options for next session** (in plain-English outcome order):

1. **Refresh action sheet from #62 (now-current plan)**. One-page handout for "what to do this week" based on the approved plan; the FM's 8 tracked refinements become practical next-step items. Existing sheet at `docs/handovers/2026-06-01-act-on-current-plan.md` was written against #61's content; refresh against #62 + the 4 accepted deltas (US-situs $1.37M target, FIRE-bridge analysis 49→60→67, broad-market multiple-compression risk theme, parameterized UCITS top-up trigger). Now actionable since the recap surfaces the data visually.
2. **Wave 8 v2 follow-ons** (per above list — glidepath label matching / integration render test / cashflow slider integration). All non-blocking.
3. **Wave 7 Piece B v1.5 — post-FM auto-populate** (unchanged). When the next synthesis FM verdict commits, run the carry-forward matcher against the new draft's objections + write `fm_objection_user_state` rows with the prior stance + counter-position pre-populated. /plan loads with the matched-subset stances ready.
4. **Wave 7 Piece A — triage agent** (unchanged). New agent classifies FM objections into auto_dispatch / defined_fix / human_in_loop / invalid_objection / dismissed. ~3-4 days. Less urgent now that #62's FM verdict showed self-convergence via prompt-only guidance.
5. **Wave 6 — enforcement substrate (FM-OBJ #7 fix)** (unchanged). Typed `PlanPolicy` with `TickerCapRule` / `SectorCapRule` / `ExposureCapRule`; ~1.5-2 weeks per codex zigzag.

**Previous (early evening, wave 8 Piece A only):** post-Accept-All /plan routing fix — explicit five-state discriminator replaces the implicit JSX fall-through that landed users on the rejected #61 draft view after #62 was accepted. Branch `main` @ `0cc3dc8`. See the Piece A row above for the per-piece breakdown.

**Previous (late evening, wave 7 Piece B):** prime directive in FM prompts + carry-forward end-to-end + first APPROVED plan since 2026-05-29 (synth #62) + user pressed Accept All → plan_version #19 promoted to canonical current. Branch `main` @ `cad7779`. Five commits past the wave-5 close (`7835226`):
...
**Post-Accept-All UX gap — FIXED by wave 8 Piece A (this commit).** The /plan page used to fall back to `plan_version=18` (synth #61's `fm-rejected` draft) when there was no pending draft, surfacing two confusing messages: *"Showing the last completed draft — superseded by a later synthesis attempt that did not produce a fresh draft"* and *"Draft #18 · loading verdict…"*. The implicit JSX conditionals didn't have a "current plan exists, no pending draft → show recap" branch. Piece A introduces an explicit five-state discriminator (`derivePlanViewState` in `ui/src/lib/plan-view-state.ts`) returning one of `{ no_plan, pending_draft_triage, in_flight_synthesis, recap_current, stale_fallback_with_warning }`; every render block in `page.tsx` now gates on it. The exact #61/#62 scenario (plan=19 + draft fallback to #18 superseded + no in-flight) now lands on `recap_current`, which renders a minimal "Your plan is current" placeholder + the plan markdown. Stale banner only fires under `stale_fallback_with_warning` (no current plan). 12 unit tests pin the matrix including the #61/#62 regression. **Recap content is intentionally sparse** — Piece A is the plumbing, Pieces E (markdown rendering) / G (headline + recap-summary) / B (allocation glidepath) / F (actions timeline) / C (cashflow defaults) / D (Monte Carlo) fill out the recap layout per the wave-8 ship order.

**Pickup options for next session** (in plain-English outcome order — wave 8 is the user's stated new priority, Piece A landed; Piece E is the next smallest):

1. **★ Wave 8 Piece E — Markdown rendering for horizon_*_md**. Smallest remaining piece (~half day). Today the recap placeholder dumps `plan.raw_markdown` via `<Markdown>` but the per-horizon markdown fields on `DraftResponse` (`horizon_long_md` / `horizon_medium_md` / `horizon_short_md`) are still rendered in `<pre>` tags in the off-canvas PlanRevisionSheet. Lift the shared `<Markdown>` component into the recap's "Full plan" collapsible so the formatted plan reads like prose, not a code dump. Sanity-check XSS / arbitrary-HTML behaviour of the markdown renderer (`react-markdown` should be safe by default but verify with one test). Outcome: the recap view has formatted long/medium/short markdown the moment Piece E lands — no chart pieces needed yet. **This is the next ship per wave-8 order A → E → G → B1 → F → B2 → C → D.**

2. **Wave 8 Piece G — headline + recap-summary service**. New backend helper `argosy/services/plan_headline.py` covering the three-line headline (retirement readiness / next big move / then) plus the four at-a-glance blocks (accepted deltas / portfolio value / insurance gaps / audit line). New route `GET /api/plan/current/headline`. UI `HeadlineCard` at top of recap. Estimated ~1 day. Outcome: a non-expert opens /plan and immediately sees the plain-English bottom line. After G the recap has its at-a-glance layer; charts (B/F/C/D) compose in.

3. **Remaining wave-8 pieces in ship order**: B1 (allocation glidepath backend service — ~2-3 days, biggest overrun risk per codex), B2 (allocation glidepath chart, ~1 day), F (actions timeline, ~1 day), C (cashflow defaults v1, ~1 day), D (Monte Carlo on recap, ~1 day). Plan doc at `docs/superpowers/plans/2026-06-01-wave-8-plan-recap-view.md` (rev 4, ZIGZAG CONVERGED — commit `f8e294d`).

4. **Wave 8 Piece A follow-up — integration render test**. Codex's MINOR #1 from the Piece A review: add a smoke test that renders `<PlanPage />` with mocked API responses and asserts `data-view-state="recap_current"` (and the matching sub-component presence/absence) for the #61/#62 scenario. Requires mocking 7 API methods + `useWSEvents` + `useRouter`. Pure-function discriminator already has 12 tests; this would close the "pins which sub-components render" wording in the spec literally. Cheap to ship alongside Piece E or G when there's more recap content to assert against.

5. **Refresh action sheet from #62 (now-current plan)**. One-page handout for "what to do this week" based on the approved plan; the FM's 8 tracked refinements become practical next-step items. Outcome: actionable today; doesn't require another synth loop. Existing sheet at `docs/handovers/2026-06-01-act-on-current-plan.md` was written against #61's content — may want a refresh against #62 + the 4 deltas Ariel just accepted (US-situs $1.37M target, FIRE-bridge analysis 49→60→67, broad-market multiple-compression risk theme, parameterized UCITS top-up trigger). Lower priority while wave 8 is in flight.

6. **Wave 7 Piece B v1.5 — post-FM auto-populate**. (Unchanged from previous handover.) When the next synthesis FM verdict commits, run the carry-forward matcher (already built + tested) against the new draft's objections + write `fm_objection_user_state` rows with the prior stance + counter-position + audit fields. /plan loads with stances pre-populated for the matched subset.

7. **Wave 7 Piece A — triage agent (central wave-7 piece)**. (Unchanged.) New agent runs after phase 5; classifies each FM objection into auto_dispatch / defined_fix / human_in_loop / invalid_objection / dismissed. ~3-4 days.

8. **Wave 6 — enforcement substrate (FM-OBJ #7 fix)**. (Unchanged.) Typed `PlanPolicy` with `TickerCapRule`/`SectorCapRule`/`ExposureCapRule`; ~1.5-2 weeks per codex zigzag.

**Original wave-8 scoping entry (kept for reference, now landing piece by piece):** Visual-first recap composing existing visualizations (NVDA trajectory chart + cashflow projection + Monte Carlo + ruin probability + target progress strips + 13 retirement cards) into a single "here's your plan" surface. Eight pieces (A post-accept routing **— LANDED** / B1 allocation glidepath backend / B2 chart / C cashflow defaults with rationale / D Monte Carlo integration / E markdown rendering / F cross-horizon actions timeline / G plain-English headline). Plan doc at `docs/superpowers/plans/2026-06-01-wave-8-plan-recap-view.md` (rev 4, commit `f8e294d`); **codex zigzag CONVERGED after 3 rounds**. Ship order **A → E → G → B1 → F → B2 → C → D** so the page has something readable from day 1 and charts compose in.
