...
- Commit #17 (pending): Home Red-Flag Strip UI + backend `/monitor/flags/{id}/acknowledge` route. The UI fetches active flags on mount (kind-aware summary line per flag: drift / mc_regression / macro_shift), renders severity dot + dismiss button, optimistically removes on click + POSTs to acknowledge. Empty state renders nothing (zero vertical space). Backend route refactor: `/monitor/flags` now returns rows directly from `monitor_flags` (not via `to_dict()` of dataclasses) — exposes id, surfaced_at, expires_at, payload-as-object for the UI's acknowledge wiring. mc_regression rows where `payload['fired']` is false (baselines / no-fire anchors) are filtered out at the route layer. 8 new tests in test_monitor_flags_routes.py pass.
