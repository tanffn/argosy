...
- Commit #8 (pending): `/life-events` page + agent + Pydantic enum validator + 422 banner UI. Loud-error contract verified by codex APPROVE_WITH_CONDITIONS — 2 IMPORTANTs addressed: (a) typed `InvalidKindForCategoryError` exception replaces the brittle `"kind=" in msg` string-match discriminator (route now catches the typed class explicitly, no Pydantic-message-format coupling), (b) `field_rules_by_category` added to catalog response so UI doesn't hardcode which categories need amount / recurring_years — server-driven via `requires_amount` + `supports_recurring_years` flags. UI banner renders LOCALLY in the form component (not via global error boundary) per the BLOCKER spec contract. 11 backend route tests + sub-agent-delivered UI page + nav entry + api.ts client. TypeScript clean. Catalog endpoint exposes the 6 categories + per-category kind enum + per-category field rules.
