...
Idle cash is the primary action signal: the same pool of free cash is read three ways, by three different engines, presented on `/proposals` as ONE section — `deploy-cash-flow` in `ui/src/app/proposals/page.tsx`, headed **"Deploy your cash."** A user reads one flow, not three unrelated cards. The three cards are unchanged internals under a shared header and intro; the `#deploy-cash` and `#allocation` anchors are preserved as scroll targets inside the section so Home-banner and intra-page deep-links still resolve.

- **How much is deployable** — the continuous overage detector (`unallocated_cash_detector.py`, rendered by `UnallocatedCashCard`). Self-tuning: it fires when current cash exceeds the plan-target cash row by ~1.5× and renders nothing when there is no overage. This is the "free cash above plan target → here's where it could go" signal.
- **Where it goes** — the deployment advisor's tiered, estate-annotated BUY list (`deployment_advisor.py`, rendered by `DeployCashCard`). Prefilled from the detected idle cash; the user can adjust the amount and regenerate. Each line carries an `is_new` flag — new vs already-held in the aggregate tradeable book — surfaced as a **NEW / ADD** tag.
- **The horizon split** — the event-driven reconciliation (`WindfallCard`). Self-suppresses (renders nothing) unless a cash-position change is detected; when it fires it shows the long/medium/short horizon reconciliation with its source breakdown.
