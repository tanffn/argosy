...
- Commits #11+#12 (pending, bundled): monitor agent drift trigger + monthly MC regression trigger in `argosy/services/plan_monitor.py`. Drift uses hysteresis contract `(rel ≥ 10% AND consecutive ≥ 2 distinct snapshot dates) OR (rel ≥ 20% single-shot)` AND `abs ≥ $5K`, severity bands info/warning/critical at the 0.10/0.20/0.30 thresholds. MC regression runs project_monte_carlo, compares P(solvent at 95) month-over-month, fires when delta ≤ -5pp. Codex APPROVE_WITH_CONDITIONS — 1 BLOCKER + 1 IMPORTANT addressed: (a) hysteresis was counting raw monitor_flags rows which over-counted on correction-reruns; rewrote `_count_prior_distinct_drift_snapshots()` to dedupe by `payload['snapshot_date']` + exclude the current snapshot so re-runs of the same snapshot can never satisfy the persistent path; new regression test pins this behavior; (b) MC `/monitor/mc-check` route now enforces a 25-day cooldown — returns cached payload from the most recent prior run unless `force=true` (cron uses force=true; UI manual triggers don't, preventing accidental-loop CPU burn). 18 tests pass (10 drift + 7 MC + 1 new correction-rerun guard).
