...
- Commit #5 (pending): `/decide` → `/consult` rename. The old name was misleading — the page is a ticker consultation surface (decisions happen on /proposals). All cross-references updated in one commit (no broken-nav window per codex IMPORTANT on spec #1): `ui/src/app/decide/page.tsx` → `ui/src/app/consult/page.tsx` (function `DecidePage` → `ConsultPage`, h1 + heading text + run-button label updated, "consult" replaces "decision" in the description text); nav.tsx tab label + href + flow comment; proposals/page.tsx empty-state CTA href + label; next.config.ts adds 308 permanent redirect for `/decide` + `/decide/*` → `/consult` + `/consult/*` for old bookmarks; user-guide refs updated throughout (TOC, primary tab list, tab summary table, §15 heading, figcaption, audit table row). TypeScript check clean post-rename.
