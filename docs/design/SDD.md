...
- Commit #9 (pending): canonical `effective_retire_ready_age()` in `cashflow_projection.py` (closes codex BLOCKER #3 from spec #1 review — no consumer computes retire-ready-age independently of this function). New `EffectiveRetireReadyAge` dataclass + helper `_latest_projected_rsu_vest_date()`. Clamps in order: (1) base projection from existing `project_cashflow` + `detect_retire_ready`, (2) RSU vest clamp using a heuristic — latest historical vest + 90 days projects next-expected vest, iterated forward in 90-day steps until past `as_of` (good for users on quarterly vests like NVDA employees; over-conservative for annual-vest users; replaced by grant-aware projection in commit #10 follow-on), (3) life-event clamp stubbed (returns None pending /life-events service-layer query in commit #8 — the hook is in the function from day 1 so consumers don't need to migrate later). 7 new tests verify shape + all three scenarios + same-inputs-same-output invariant. Consumer migration (ExpectedRetirementAgeCard, RuinProbabilityHero, monitor agent MC regression all calling this function) deferred to a follow-on — this commit lands the function + its tests, not the migration of all call sites.
