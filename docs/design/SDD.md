...
- Commit #15 (pending): monitor macro-shift trigger. New `check_macro_shift()` + `get_active_macro_shift_flags()` in `argosy/services/plan_monitor.py`. Reads news_signals where materiality='high' AND recommended_flag='macro_shift' AND analyzed_at IS NOT NULL AND received_at within lookback (7d default). Idempotent: each news_signal_id fires at most once via `_already_flagged_news_signal_ids()`. Severity = critical when event_keywords intersect with {war, sanction, geopolitical, Taiwan, FOMC}; warning otherwise. Per-spec marked "wiring only" so no codex zigzag. 11 new tests pass.
