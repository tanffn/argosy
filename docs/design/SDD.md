...
- Commit #3 (pending): migration 0042 — new `life_events` table backing the structured-intake page (commit #8). 11 columns (id, user_id FK, category enum, kind, target_date, amount_usd, recurring_years, description, source_id, created_at, updated_at). Category enum (6 values) enforced via CHECK constraint at DB layer; per-category kind enum enforced by Pydantic at service layer (commit #8). Indexes on (user_id, target_date) and (user_id, category). Codex review running.
