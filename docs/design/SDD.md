...
- Commit #6b (pending): UnallocatedCashCard Accept/Defer wiring + new generic `/api/proposals/allocation/{accept,defer,actions}` routes. Per Ariel's design-phase Q4 answer ("Log the data, I will manually buy"), this is intent-persistence only — no auto-promotion to /proposals trade execution. The new `argosy/api/routes/allocation.py` generalizes the windfall pattern over the `action_source` discriminator from migration 0041 (accepts all 6 enum values: windfall / unallocated_cash / monitor_drift / rebalance / life_event / manual). Pydantic `Literal` validation surfaces unknown action_source as 422 before the DB CHECK fires. UnallocatedCashCard now: (a) fetches existing actions on mount, (b) renders Accept/Defer buttons per proposal row, (c) builds a JSON `source_ref` from `{snapshot_date, horizon, asset_class, instrument}` so the partial unique index dedups exact-same-proposal re-clicks while allowing distinct proposals from the same snapshot to coexist, (d) flips to a "✓ Accepted" / "↻ Deferred" pill after click. The existing /retirement/windfall/{accept,defer,actions} routes continue to work (backward compat); both write to the same allocation_actions table. 7 new tests in `test_allocation_routes.py`; existing 22 tests (test_unallocated_cash_detector + test_windfall_actions) still pass.
