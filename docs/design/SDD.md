...
- Sprint #2 commits #5, #6+#7, #8+#9 (pending — bundled 5 detector buckets in 3 commits): 5 anomaly detectors written by 3 parallel sub-agents. Bucket A (amount outliers — median+MAD robust stats), Bucket B (watchlist 4-state machine + recurring-pattern learner), Bucket C (novel-merchant + category-drift), Bucket D (cross-card duplicate). All write to `expense_review_queue` with bucket-specific dedup_keys per spec §4. Codex tandem review on Buckets A+B returned BLOCK initially — dedup_key literals were hardcoded (`thr:3|min:200`), would silently suppress fires after threshold tweaks. Fixed: dedup_key now interpolates `A1_Z_THRESHOLD` + `A1_MIN_ABS_AMOUNT_NIS` + `A2_MIN_MULTIPLE` constants verbatim. Also: B2 auto-dormancy added — after 2 missed cycles, status flips to 'dormant' so cancelled subscriptions stop firing forever. 48 tests pass across all 5 buckets.
