...
| Snapshot self-refresh / apply broker fills (TSV is OUTPUT-only) | `argosy/services/snapshot_refresh.py` (`refresh_portfolio_snapshot`, `apply_fills_to_snapshot`) + `argosy/services/jobs/snapshot_refresh_job.py` | §20.4; `argosy/services/portfolio_ingest/tsv_generator.py` (`generate_family_finances_tsv`); job registered `enabled=False` in `argosy/api/main.py` (manual Run-now) |
