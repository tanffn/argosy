...
1. **★ Wave 8 — Plan recap view ("what's the plan, in plain English")**. User-stated priority after the Accept-All UX gap surfaced (synth #62 approved, plan_version=19 promoted, but /plan falls back to confusing stale draft view). Visual-first recap composing existing visualizations (NVDA trajectory chart + cashflow projection + Monte Carlo + ruin probability + target progress strips + 13 retirement cards) into a single "here's your plan" surface. Six pieces (A post-accept routing / B allocation glidepath / C synthesizer-aware cashflow defaults with rationale / D Monte Carlo integration / E markdown rendering / F cross-horizon actions timeline / G plain-English headline). Plan doc at `docs/superpowers/plans/2026-06-01-wave-8-plan-recap-view.md` (rev 2, commit `b8a06eb`); codex zigzag round 1 returned `SCOPE-CHANGES-NEEDED` and all 7 findings applied (Piece B split into B1 backend + B2 UI chart; Piece C narrowed to v1 deterministic — no synthesizer-posture string interpretation; direction-reversal guardrail + non-% exclusion filter added to B1; explicit five-state discriminator on Piece A with one test per branch; four at-a-glance blocks added to layout per codex's "missing in recap"). Round 2 confirmation pending. Outcome: a non-expert can look at /plan and understand what the plan IS — current allocation → planned glidepath over time, NVDA share trajectory, monthly cashflow with pre-populated sliders + rationale tooltips, Monte Carlo verdict, key-actions timeline, formatted plan markdown. ~1.5-2 weeks per timeline in the doc. **This is the next critical task per the user.**
