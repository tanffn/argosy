...
**Home page — `<DecisionAccordion>`** (`ui/src/components/decision-accordion.tsx`). Replaces the former flat agent-activity firehose on `/`. Each row represents one `decision_run`; clicking it expands to reveal the ordered cascade of `<AgentRunCard>` rows for that decision. Data comes from `/api/decisions/recent` (polling interval 30 s). Empty state: "No decisions yet." The accordion is rendered below the hero stats and above the briefs section.
...
**`useDecisionStream` hook** (`ui/src/hooks/use-decision-stream.ts`). Owns the WS connection + REST merge. Subscribes to `agent.run.*` events on the shared WS; deduplicates against already-fetched `agent_reports` rows using `run_correlation_id`. WS-only rows (not yet flushed to DB) are held in local state and merged with the DB payload on the next poll cycle. The hook also applies the per-user filter that the global WS broadcast lacks (§15.4 known issue): events are accepted only when `event.user_id === currentUserId`. Other pages that consume the raw WS still inherit the original cross-user quirk.

**Known limitation.** The WS-only → DB-row merge heuristic in `useDecisionStream` matches on `agent_role + decision_id + ±10 s` timestamp when `run_correlation_id` is absent (older emitters). Parallel runs of the same agent within one decision (rare but theoretically possible) can mis-link a WS card to the wrong DB row. Once all emitters are on the new `BaseAgent.run()` path the `run_correlation_id` is the sole join key and the heuristic is bypassed.
